...
def crawl(
    ...
    chrome_options.add_argument('--proxy-server=http://localhost:{}'.format(proxy_port))
    ...
    try:
        print("[Selenium Proxy] Loading %s" % url)
        driver.get(url)
...
def crawl_no_proxy(
    ...
    try:
        print("[Selenium] Loading %s" % page_source_fpath)
        driver.get(page_source_fpath)
        
        if len(driver.page_source) < 100:
            return
        
        print("final_url : {}".format(driver.current_url))
...
HOME_DIR = "/yopo-artifact/data"
rendering_stream_dir = HOME_DIR + '/rendering_stream/'
driver_path = '/yopo-artifact/A4/AdGraphAPI/chromedriver' 
# tested with ChromeDriver version 2.42
binary_path = '/yopo-artifact/A4/AdGraphAPI/AdGraph-Ubuntu-16.04/chrome'
log_extraction_script = "document.createCDATASection('NOTVERYUNIQUESTRING');"

domain = args.domain
print("[Proxy] Loading URL: %s | %s" % (domain, args.final_domain))
...
if args.mode == 'proxy':
    if args.load_modified:
        if int(args.id) % 15 == 0:
            ...
            raise("Wrong crawler id!")
    else:
        if args.weirdo:
            crawl(domain, driver_path, binary_path, log_extraction_script, 6668)
        else:
            if int(args.id) % 2 == 0:
                crawl(domain, driver_path, binary_path, log_extraction_script, 6666)
            elif int(args.id) % 2 == 1:
                crawl(domain, driver_path, binary_path, log_extraction_script, 6667)
                
elif args.mode == 'no_proxy':
    page_source_fpath = args.domain
    url_mapping_fpath = HOME_DIR + '/rendering_stream/original_domain_to_final_url_mapping_%s.csv' % args.id
