// Autogenerated by the ProtoZero compiler plugin. DO NOT EDIT.

#ifndef PERFETTO_PROTOS_PERFETTO_TRACE_FTRACE_BINDER_TRANSACTION_RECEIVED_PROTO_H_
#define PERFETTO_PROTOS_PERFETTO_TRACE_FTRACE_BINDER_TRANSACTION_RECEIVED_PROTO_H_

#include <stddef.h>
#include <stdint.h>

#include "perfetto/protozero/proto_field_descriptor.h"
#include "perfetto/protozero/message.h"

namespace perfetto {
namespace protos {
namespace pbzero {


class BinderTransactionReceivedFtraceEvent : public ::protozero::Message {
 public:
  enum : int32_t {
    kDebugIdFieldNumber = 1,
  };
  static const ::protozero::ProtoFieldDescriptor* GetFieldDescriptor(uint32_t field_id);
  void set_debug_id(int32_t value) {
    AppendVarInt(1, value);
  }
};

} // Namespace.
} // Namespace.
} // Namespace.
#endif  // Include guard.
