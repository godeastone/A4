...
class EpubTextEmphasis final : public CSSUnresolvedProperty {
 public:
  ...
  const char* GetPropertyName() const override { return "-epub-text-emphasis\0"; }
  const WTF::AtomicString& GetPropertyNameAtomicString() const override {
    static const WTF::AtomicString& name = WTF::AtomicString("-epub-text-emphasis\0");
    return name;
  }
  const WTF::String GetJSPropertyName() const override {
    return WTF::String("epubTextEmphasis\0");
  }
};
