...
class CaptionSide final : public Longhand {
 public:
  constexpr CaptionSide() : Longhand() {}
  const char* GetPropertyName() const override { return "caption-side\0"; }
  const WTF::AtomicString& GetPropertyNameAtomicString() const override {
    static const WTF::AtomicString& name = WTF::AtomicString("caption-side\0");
    return name;
  }
  const WTF::String GetJSPropertyName() const override {
    return WTF::String("captionSide\0");
  }
  CSSPropertyID PropertyID() const override { return CSSPropertyCaptionSide; }
  ...
  void ApplyValue(StyleResolverState& state, const CSSValue& value) const override {
    state.Style()->SetCaptionSide(ToCSSIdentifierValue(value).ConvertTo<blink::ECaptionSide>());
    state.Style()->SetCaptionSideIsInherited(false);
  }
};
