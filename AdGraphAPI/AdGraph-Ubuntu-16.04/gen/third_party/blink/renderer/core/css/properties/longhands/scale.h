...
class Scale final : public Longhand {
 public:
  constexpr Scale() : Longhand() {}
  bool IsEnabled() const override {
    return RuntimeEnabledFeatures::CSSIndependentTransformPropertiesEnabled();
  }
  const char* GetPropertyName() const override { return "scale\0"; }
  const WTF::AtomicString& GetPropertyNameAtomicString() const override {
    static const WTF::AtomicString& name = WTF::AtomicString("scale\0");
    return name;
  }
  const WTF::String GetJSPropertyName() const override {
    return WTF::String("scale\0");
  }
  CSSPropertyID PropertyID() const override { return CSSPropertyScale; }
