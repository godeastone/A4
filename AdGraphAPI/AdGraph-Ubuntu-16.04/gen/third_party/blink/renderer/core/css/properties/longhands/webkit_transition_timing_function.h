...
class WebkitTransitionTimingFunction final : public CSSUnresolvedProperty {
 public:
  ...
  const char* GetPropertyName() const override { return "-webkit-transition-timing-function\0"; }
  const WTF::AtomicString& GetPropertyNameAtomicString() const override {
    static const WTF::AtomicString& name = WTF::AtomicString("-webkit-transition-timing-function\0");
    return name;
  }
  const WTF::String GetJSPropertyName() const override {
    return WTF::String("webkitTransitionTimingFunction\0");
  }
};
