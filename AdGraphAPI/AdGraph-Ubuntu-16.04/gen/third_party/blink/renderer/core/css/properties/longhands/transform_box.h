...
class TransformBox final : public Longhand {
 public:
  constexpr TransformBox() : Longhand() {}
  bool IsEnabled() const override {
    return RuntimeEnabledFeatures::CSSTransformBoxEnabled();
  }
  const char* GetPropertyName() const override { return "transform-box\0"; }
  const WTF::AtomicString& GetPropertyNameAtomicString() const override {
    static const WTF::AtomicString& name = WTF::AtomicString("transform-box\0");
    return name;
  }
  const WTF::String GetJSPropertyName() const override {
    return WTF::String("transformBox\0");
  }
  CSSPropertyID PropertyID() const override { return CSSPropertyTransformBox; }
  ...
  void ApplyValue(StyleResolverState& state, const CSSValue& value) const override {
    state.Style()->SetTransformBox(ToCSSIdentifierValue(value).ConvertTo<blink::ETransformBox>());
  }
};
