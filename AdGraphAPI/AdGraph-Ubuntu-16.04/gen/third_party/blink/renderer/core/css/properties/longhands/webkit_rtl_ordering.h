...
class WebkitRtlOrdering final : public Longhand {
 public:
  constexpr WebkitRtlOrdering() : Longhand() {}
  const char* GetPropertyName() const override { return "-webkit-rtl-ordering\0"; }
  const WTF::AtomicString& GetPropertyNameAtomicString() const override {
    static const WTF::AtomicString& name = WTF::AtomicString("-webkit-rtl-ordering\0");
    return name;
  }
  const WTF::String GetJSPropertyName() const override {
    return WTF::String("webkitRtlOrdering\0");
  }
  CSSPropertyID PropertyID() const override { return CSSPropertyWebkitRtlOrdering; }
  ...
  void ApplyValue(StyleResolverState& state, const CSSValue& value) const override {
    state.Style()->SetRtlOrdering(ToCSSIdentifierValue(value).ConvertTo<blink::EOrder>());
    state.Style()->SetRtlOrderingIsInherited(false);
  }
};
