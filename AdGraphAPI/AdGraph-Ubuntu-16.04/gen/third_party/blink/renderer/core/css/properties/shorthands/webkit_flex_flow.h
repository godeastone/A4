...
class WebkitFlexFlow final : public CSSUnresolvedProperty {
 public:
  ...
  const char* GetPropertyName() const override { return "-webkit-flex-flow\0"; }
  const WTF::AtomicString& GetPropertyNameAtomicString() const override {
    static const WTF::AtomicString& name = WTF::AtomicString("-webkit-flex-flow\0");
    return name;
  }
  const WTF::String GetJSPropertyName() const override {
    return WTF::String("webkitFlexFlow\0");
  }
};
