...
class MaskType final : public Longhand {
 public:
  constexpr MaskType() : Longhand() {}
  const char* GetPropertyName() const override { return "mask-type\0"; }
  const WTF::AtomicString& GetPropertyNameAtomicString() const override {
    static const WTF::AtomicString& name = WTF::AtomicString("mask-type\0");
    return name;
  }
  const WTF::String GetJSPropertyName() const override {
    return WTF::String("maskType\0");
  }
  CSSPropertyID PropertyID() const override { return CSSPropertyMaskType; }
  ...
  void ApplyValue(StyleResolverState& state, const CSSValue& value) const override {
    state.Style()->AccessSVGStyle().SetMaskType(ToCSSIdentifierValue(value).ConvertTo<blink::EMaskType>());
  }
};
