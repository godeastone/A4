...
class Hyphens final : public Longhand {
 public:
  constexpr Hyphens() : Longhand() {}
  const char* GetPropertyName() const override { return "hyphens\0"; }
  const WTF::AtomicString& GetPropertyNameAtomicString() const override {
    static const WTF::AtomicString& name = WTF::AtomicString("hyphens\0");
    return name;
  }
  const WTF::String GetJSPropertyName() const override {
    return WTF::String("hyphens\0");
  }
  CSSPropertyID PropertyID() const override { return CSSPropertyHyphens; }
  ...
  void ApplyValue(StyleResolverState& state, const CSSValue& value) const override {
    state.Style()->SetHyphens(ToCSSIdentifierValue(value).ConvertTo<blink::Hyphens>());
  }
};
