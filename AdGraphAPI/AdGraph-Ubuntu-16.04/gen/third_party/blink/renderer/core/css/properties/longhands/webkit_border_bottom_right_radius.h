...
class WebkitBorderBottomRightRadius final : public CSSUnresolvedProperty {
 public:
  ...
  const char* GetPropertyName() const override { return "-webkit-border-bottom-right-radius\0"; }
  const WTF::AtomicString& GetPropertyNameAtomicString() const override {
    static const WTF::AtomicString& name = WTF::AtomicString("-webkit-border-bottom-right-radius\0");
    return name;
  }
  const WTF::String GetJSPropertyName() const override {
    return WTF::String("webkitBorderBottomRightRadius\0");
  }
};
