...
class StrokeLinejoin final : public Longhand {
 public:
  constexpr StrokeLinejoin() : Longhand() {}
  const char* GetPropertyName() const override { return "stroke-linejoin\0"; }
  const WTF::AtomicString& GetPropertyNameAtomicString() const override {
    static const WTF::AtomicString& name = WTF::AtomicString("stroke-linejoin\0");
    return name;
  }
  const WTF::String GetJSPropertyName() const override {
    return WTF::String("strokeLinejoin\0");
  }
  CSSPropertyID PropertyID() const override { return CSSPropertyStrokeLinejoin; }
  ...
  void ApplyValue(StyleResolverState& state, const CSSValue& value) const override {
    state.Style()->AccessSVGStyle().SetJoinStyle(ToCSSIdentifierValue(value).ConvertTo<blink::LineJoin>());
  }
};
