...
class FlexGrow final : public Longhand {
 public:
  constexpr FlexGrow() : Longhand() {}
  const char* GetPropertyName() const override { return "flex-grow\0"; }
  const WTF::AtomicString& GetPropertyNameAtomicString() const override {
    static const WTF::AtomicString& name = WTF::AtomicString("flex-grow\0");
    return name;
  }
  const WTF::String GetJSPropertyName() const override {
    return WTF::String("flexGrow\0");
  }
  CSSPropertyID PropertyID() const override { return CSSPropertyFlexGrow; }
