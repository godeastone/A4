...
class FlexFlow final : public Shorthand {
 public:
  constexpr FlexFlow() : Shorthand() {}
  const char* GetPropertyName() const override { return "flex-flow\0"; }
  const WTF::AtomicString& GetPropertyNameAtomicString() const override {
    static const WTF::AtomicString& name = WTF::AtomicString("flex-flow\0");
    return name;
  }
  const WTF::String GetJSPropertyName() const override {
    return WTF::String("flexFlow\0");
  }
  CSSPropertyID PropertyID() const override { return CSSPropertyFlexFlow; }
