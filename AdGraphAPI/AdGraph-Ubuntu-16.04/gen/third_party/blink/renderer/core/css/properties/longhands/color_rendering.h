...
class ColorRendering final : public Longhand {
 public:
  constexpr ColorRendering() : Longhand() {}
  const char* GetPropertyName() const override { return "color-rendering\0"; }
  const WTF::AtomicString& GetPropertyNameAtomicString() const override {
    static const WTF::AtomicString& name = WTF::AtomicString("color-rendering\0");
    return name;
  }
  const WTF::String GetJSPropertyName() const override {
    return WTF::String("colorRendering\0");
  }
  CSSPropertyID PropertyID() const override { return CSSPropertyColorRendering; }
  ...
  void ApplyValue(StyleResolverState& state, const CSSValue& value) const override {
    state.Style()->AccessSVGStyle().SetColorRendering(ToCSSIdentifierValue(value).ConvertTo<blink::EColorRendering>());
  }
};
