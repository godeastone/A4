...
class BreakBefore final : public Longhand {
 public:
  constexpr BreakBefore() : Longhand() {}
  const char* GetPropertyName() const override { return "break-before\0"; }
  const WTF::AtomicString& GetPropertyNameAtomicString() const override {
    static const WTF::AtomicString& name = WTF::AtomicString("break-before\0");
    return name;
  }
  const WTF::String GetJSPropertyName() const override {
    return WTF::String("breakBefore\0");
  }
  CSSPropertyID PropertyID() const override { return CSSPropertyBreakBefore; }
  ...
  void ApplyValue(StyleResolverState& state, const CSSValue& value) const override {
    state.Style()->SetBreakBefore(ToCSSIdentifierValue(value).ConvertTo<blink::EBreakBetween>());
  }
};
