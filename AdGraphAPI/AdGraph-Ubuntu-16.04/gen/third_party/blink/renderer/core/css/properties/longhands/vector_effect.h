...
class VectorEffect final : public Longhand {
 public:
  constexpr VectorEffect() : Longhand() {}
  const char* GetPropertyName() const override { return "vector-effect\0"; }
  const WTF::AtomicString& GetPropertyNameAtomicString() const override {
    static const WTF::AtomicString& name = WTF::AtomicString("vector-effect\0");
    return name;
  }
  const WTF::String GetJSPropertyName() const override {
    return WTF::String("vectorEffect\0");
  }
  CSSPropertyID PropertyID() const override { return CSSPropertyVectorEffect; }
  ...
  void ApplyValue(StyleResolverState& state, const CSSValue& value) const override {
    state.Style()->AccessSVGStyle().SetVectorEffect(ToCSSIdentifierValue(value).ConvertTo<blink::EVectorEffect>());
  }
};
