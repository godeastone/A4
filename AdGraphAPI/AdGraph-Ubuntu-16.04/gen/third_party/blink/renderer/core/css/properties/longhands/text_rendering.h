...
class TextRendering final : public Longhand {
 public:
  constexpr TextRendering() : Longhand() {}
  const char* GetPropertyName() const override { return "text-rendering\0"; }
  const WTF::AtomicString& GetPropertyNameAtomicString() const override {
    static const WTF::AtomicString& name = WTF::AtomicString("text-rendering\0");
    return name;
  }
  const WTF::String GetJSPropertyName() const override {
    return WTF::String("textRendering\0");
  }
  CSSPropertyID PropertyID() const override { return CSSPropertyTextRendering; }
  ...
  void ApplyValue(StyleResolverState& state, const CSSValue& value) const override {
    state.GetFontBuilder().SetTextRendering(ToCSSIdentifierValue(value).ConvertTo<blink::TextRenderingMode>());
  }
};
