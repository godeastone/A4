...
class Clear final : public Longhand {
 public:
  constexpr Clear() : Longhand() {}
  const char* GetPropertyName() const override { return "clear\0"; }
  const WTF::AtomicString& GetPropertyNameAtomicString() const override {
    static const WTF::AtomicString& name = WTF::AtomicString("clear\0");
    return name;
  }
  const WTF::String GetJSPropertyName() const override {
    return WTF::String("clear\0");
  }
  CSSPropertyID PropertyID() const override { return CSSPropertyClear; }
  ...
  void ApplyValue(StyleResolverState& state, const CSSValue& value) const override {
    state.Style()->SetClear(ToCSSIdentifierValue(value).ConvertTo<blink::EClear>());
  }
};
