...
class BorderRightStyle final : public Longhand {
 public:
  constexpr BorderRightStyle() : Longhand() {}
  const char* GetPropertyName() const override { return "border-right-style\0"; }
  const WTF::AtomicString& GetPropertyNameAtomicString() const override {
    static const WTF::AtomicString& name = WTF::AtomicString("border-right-style\0");
    return name;
  }
  const WTF::String GetJSPropertyName() const override {
    return WTF::String("borderRightStyle\0");
  }
  CSSPropertyID PropertyID() const override { return CSSPropertyBorderRightStyle; }
  ...
  void ApplyValue(StyleResolverState& state, const CSSValue& value) const override {
    state.Style()->SetBorderRightStyle(ToCSSIdentifierValue(value).ConvertTo<blink::EBorderStyle>());
  }
};
