...
class WebkitColumnRule final : public CSSUnresolvedProperty {
 public:
  ...
  const char* GetPropertyName() const override { return "-webkit-column-rule\0"; }
  const WTF::AtomicString& GetPropertyNameAtomicString() const override {
    static const WTF::AtomicString& name = WTF::AtomicString("-webkit-column-rule\0");
    return name;
  }
  const WTF::String GetJSPropertyName() const override {
    return WTF::String("webkitColumnRule\0");
  }
};
