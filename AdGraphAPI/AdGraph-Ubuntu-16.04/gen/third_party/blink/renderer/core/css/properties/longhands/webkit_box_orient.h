...
class WebkitBoxOrient final : public Longhand {
 public:
  constexpr WebkitBoxOrient() : Longhand() {}
  const char* GetPropertyName() const override { return "-webkit-box-orient\0"; }
  const WTF::AtomicString& GetPropertyNameAtomicString() const override {
    static const WTF::AtomicString& name = WTF::AtomicString("-webkit-box-orient\0");
    return name;
  }
  const WTF::String GetJSPropertyName() const override {
    return WTF::String("webkitBoxOrient\0");
  }
  CSSPropertyID PropertyID() const override { return CSSPropertyWebkitBoxOrient; }
  ...
  void ApplyValue(StyleResolverState& state, const CSSValue& value) const override {
    state.Style()->SetBoxOrient(ToCSSIdentifierValue(value).ConvertTo<blink::EBoxOrient>());
  }
};
