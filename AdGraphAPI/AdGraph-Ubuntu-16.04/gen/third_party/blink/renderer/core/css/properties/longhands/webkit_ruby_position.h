...
class WebkitRubyPosition final : public Longhand {
 public:
  constexpr WebkitRubyPosition() : Longhand() {}
  const char* GetPropertyName() const override { return "-webkit-ruby-position\0"; }
  const WTF::AtomicString& GetPropertyNameAtomicString() const override {
    static const WTF::AtomicString& name = WTF::AtomicString("-webkit-ruby-position\0");
    return name;
  }
  const WTF::String GetJSPropertyName() const override {
    return WTF::String("webkitRubyPosition\0");
  }
  CSSPropertyID PropertyID() const override { return CSSPropertyWebkitRubyPosition; }
  ...
  void ApplyValue(StyleResolverState& state, const CSSValue& value) const override {
    state.Style()->SetRubyPosition(ToCSSIdentifierValue(value).ConvertTo<blink::RubyPosition>());
  }
};
