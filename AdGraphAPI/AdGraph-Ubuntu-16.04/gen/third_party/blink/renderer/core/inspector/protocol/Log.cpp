...
std::unique_ptr<LogEntry> LogEntry::fromValue(protocol::Value* value, ErrorSupport* errors)
{
    ...
    if (argsValue) {
        errors->setName("args");
        result->m_args = ValueConversions<protocol::Array<v8_inspector::protocol::Runtime::API::RemoteObject>>::fromValue(argsValue, errors);
    }
    errors->pop();
    if (errors->hasErrors())
        return nullptr;
...
std::unique_ptr<protocol::DictionaryValue> LogEntry::toValue() const
{
    ...
    if (m_args.isJust())
        result->setValue("args", ValueConversions<protocol::Array<v8_inspector::protocol::Runtime::API::RemoteObject>>::toValue(m_args.fromJust()));
    return result;
}
