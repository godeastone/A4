...
class MODULES_EXPORT RequestDeviceOptions : public IDLDictionaryBase {
 ...
 public:
  ...
  const HeapVector<BluetoothLEScanFilterInit>& filters() const {
    DCHECK(has_filters_);
    return filters_;
  }
  void setFilters(const HeapVector<BluetoothLEScanFilterInit>&);
 ...
 private:
  ...
  HeapVector<BluetoothLEScanFilterInit> filters_;
