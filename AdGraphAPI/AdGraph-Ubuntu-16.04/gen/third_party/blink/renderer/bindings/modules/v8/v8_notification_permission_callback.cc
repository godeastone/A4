...
v8::Maybe<void> V8NotificationPermissionCallback::Invoke(ScriptWrappable* callback_this_value, const String& permission) {
...
#if DCHECK_IS_ON()
  {
    const char* valid_permission_values[] = {
        "default",
        "denied",
        "granted",
    };
    ExceptionState exception_state(GetIsolate(),
                                   ...
                                   "NotificationPermissionCallback",
                                   "invoke");
    if (!IsValidEnum(permission, valid_permission_values, arraysize(valid_permission_values), "NotificationPermission", exception_state)) {
      NOTREACHED();
      return v8::Nothing<void>();
    }
  }
  ...
  v8::Local<v8::Value> v8_permission = V8String(GetIsolate(), permission);
  v8::Local<v8::Value> argv[] = { v8_permission };
...
void V8NotificationPermissionCallback::InvokeAndReportException(ScriptWrappable* callback_this_value, const String& permission) {
  v8::TryCatch try_catch(GetIsolate());
  try_catch.SetVerbose(true);

  v8::Maybe<void> maybe_result =
      Invoke(callback_this_value, permission);
  // An exception if any is killed with the v8::TryCatch above.
  ALLOW_UNUSED_LOCAL(maybe_result);
}

MODULES_TEMPLATE_EXPORT
v8::Maybe<void> V8PersistentCallbackFunction<V8NotificationPermissionCallback>::Invoke(ScriptWrappable* callback_this_value, const String& permission) {
  return Proxy()->Invoke(
      callback_this_value, permission);
}

MODULES_TEMPLATE_EXPORT
void V8PersistentCallbackFunction<V8NotificationPermissionCallback>::InvokeAndReportException(ScriptWrappable* callback_this_value, const String& permission) {
