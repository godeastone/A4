...
v8::Maybe<void> V8SQLTransactionErrorCallback::handleEvent(ScriptWrappable* callback_this_value, SQLError* error) {
  ...
  v8::Local<v8::Value> errorHandle = ToV8(error, argument_creation_context, GetIsolate());
...
void V8SQLTransactionErrorCallback::InvokeAndReportException(ScriptWrappable* callback_this_value, SQLError* error) {
  v8::TryCatch try_catch(GetIsolate());
  try_catch.SetVerbose(true);

  v8::Maybe<void> maybe_result =
      handleEvent(callback_this_value, error);
  // An exception if any is killed with the v8::TryCatch above.
  ALLOW_UNUSED_LOCAL(maybe_result);
}

MODULES_EXTERN_TEMPLATE_EXPORT
v8::Maybe<void> V8PersistentCallbackInterface<V8SQLTransactionErrorCallback>::handleEvent(ScriptWrappable* callback_this_value, SQLError* error) {
  return Proxy()->handleEvent(
      callback_this_value, error);
}

MODULES_EXTERN_TEMPLATE_EXPORT
void V8PersistentCallbackInterface<V8SQLTransactionErrorCallback>::InvokeAndReportException(ScriptWrappable* callback_this_value, SQLError* error) {
