...
v8::Maybe<void> V8MetadataCallback::handleEvent(ScriptWrappable* callback_this_value, Metadata* metadata) {
  ...
  v8::Local<v8::Value> metadataHandle = ToV8(metadata, argument_creation_context, GetIsolate());
...
void V8MetadataCallback::InvokeAndReportException(ScriptWrappable* callback_this_value, Metadata* metadata) {
  v8::TryCatch try_catch(GetIsolate());
  try_catch.SetVerbose(true);

  v8::Maybe<void> maybe_result =
      handleEvent(callback_this_value, metadata);
  // An exception if any is killed with the v8::TryCatch above.
  ALLOW_UNUSED_LOCAL(maybe_result);
}

MODULES_EXTERN_TEMPLATE_EXPORT
v8::Maybe<void> V8PersistentCallbackInterface<V8MetadataCallback>::handleEvent(ScriptWrappable* callback_this_value, Metadata* metadata) {
  return Proxy()->handleEvent(
      callback_this_value, metadata);
}

MODULES_EXTERN_TEMPLATE_EXPORT
void V8PersistentCallbackInterface<V8MetadataCallback>::InvokeAndReportException(ScriptWrappable* callback_this_value, Metadata* metadata) {
