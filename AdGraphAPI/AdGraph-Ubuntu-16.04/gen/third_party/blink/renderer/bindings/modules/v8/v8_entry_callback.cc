...
v8::Maybe<void> V8EntryCallback::handleEvent(ScriptWrappable* callback_this_value, Entry* entry) {
  ...
  v8::Local<v8::Value> entryHandle = ToV8(entry, argument_creation_context, GetIsolate());
...
void V8EntryCallback::InvokeAndReportException(ScriptWrappable* callback_this_value, Entry* entry) {
  v8::TryCatch try_catch(GetIsolate());
  try_catch.SetVerbose(true);

  v8::Maybe<void> maybe_result =
      handleEvent(callback_this_value, entry);
  // An exception if any is killed with the v8::TryCatch above.
  ALLOW_UNUSED_LOCAL(maybe_result);
}

MODULES_EXTERN_TEMPLATE_EXPORT
v8::Maybe<void> V8PersistentCallbackInterface<V8EntryCallback>::handleEvent(ScriptWrappable* callback_this_value, Entry* entry) {
  return Proxy()->handleEvent(
      callback_this_value, entry);
}

MODULES_EXTERN_TEMPLATE_EXPORT
void V8PersistentCallbackInterface<V8EntryCallback>::InvokeAndReportException(ScriptWrappable* callback_this_value, Entry* entry) {
