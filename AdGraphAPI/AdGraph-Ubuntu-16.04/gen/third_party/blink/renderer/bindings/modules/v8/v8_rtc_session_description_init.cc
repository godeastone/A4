...
static const v8::Eternal<v8::Name>* eternalV8RTCSessionDescriptionInitKeys(v8::Isolate* isolate) {
...
void V8RTCSessionDescriptionInit::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8Value, RTCSessionDescriptionInit& impl, ExceptionState& exceptionState) {
  ...
  const v8::Eternal<v8::Name>* keys = eternalV8RTCSessionDescriptionInitKeys(isolate);
  v8::TryCatch block(isolate);
  v8::Local<v8::Context> context = isolate->GetCurrentContext();
  v8::Local<v8::Value> sdpValue;
  if (!v8Object->Get(context, keys[0].Get(isolate)).ToLocal(&sdpValue)) {
  ...
  if (sdpValue.IsEmpty() || sdpValue->IsUndefined()) {
    ...
    V8StringResource<> sdpCppValue = sdpValue;
    if (!sdpCppValue.Prepare(exceptionState))
      return;
    impl.setSdp(sdpCppValue);
  }
  ...
  if (typeValue.IsEmpty() || typeValue->IsUndefined()) {
    ...
    if (!IsValidEnum(typeCppValue, validValues, arraysize(validValues), "RTCSdpType", exceptionState))
      return;
...
bool toV8RTCSessionDescriptionInit(const RTCSessionDescriptionInit& impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  ...
  v8::Local<v8::Context> context = isolate->GetCurrentContext();
  v8::Local<v8::Value> sdpValue;
  bool sdpHasValueOrDefault = false;
  if (impl.hasSdp()) {
    sdpValue = V8String(isolate, impl.sdp());
    sdpHasValueOrDefault = true;
  }
  if (sdpHasValueOrDefault &&
      !V8CallBoolean(dictionary->CreateDataProperty(context, keys[0].Get(isolate), sdpValue))) {
    return false;
  }
