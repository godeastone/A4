...
v8::Maybe<void> V8IDBObserverCallback::Invoke(ScriptWrappable* callback_this_value, IDBObserverChanges* changes) {
  ...
  v8::Local<v8::Value> v8_changes = ToV8(changes, argument_creation_context, GetIsolate());
  v8::Local<v8::Value> argv[] = { v8_changes };
...
void V8IDBObserverCallback::InvokeAndReportException(ScriptWrappable* callback_this_value, IDBObserverChanges* changes) {
  v8::TryCatch try_catch(GetIsolate());
  try_catch.SetVerbose(true);

  v8::Maybe<void> maybe_result =
      Invoke(callback_this_value, changes);
  // An exception if any is killed with the v8::TryCatch above.
  ALLOW_UNUSED_LOCAL(maybe_result);
}

MODULES_TEMPLATE_EXPORT
v8::Maybe<void> V8PersistentCallbackFunction<V8IDBObserverCallback>::Invoke(ScriptWrappable* callback_this_value, IDBObserverChanges* changes) {
  return Proxy()->Invoke(
      callback_this_value, changes);
}

MODULES_TEMPLATE_EXPORT
void V8PersistentCallbackFunction<V8IDBObserverCallback>::InvokeAndReportException(ScriptWrappable* callback_this_value, IDBObserverChanges* changes) {
