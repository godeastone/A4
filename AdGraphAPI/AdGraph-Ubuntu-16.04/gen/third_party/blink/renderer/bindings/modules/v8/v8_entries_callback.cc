...
v8::Maybe<void> V8EntriesCallback::handleEvent(ScriptWrappable* callback_this_value, const HeapVector<Member<Entry>>& entries) {
  ...
  v8::Local<v8::Value> entriesHandle = ToV8(entries, argument_creation_context, GetIsolate());
...
void V8EntriesCallback::InvokeAndReportException(ScriptWrappable* callback_this_value, const HeapVector<Member<Entry>>& entries) {
  v8::TryCatch try_catch(GetIsolate());
  try_catch.SetVerbose(true);

  v8::Maybe<void> maybe_result =
      handleEvent(callback_this_value, entries);
  // An exception if any is killed with the v8::TryCatch above.
  ALLOW_UNUSED_LOCAL(maybe_result);
}

MODULES_EXTERN_TEMPLATE_EXPORT
v8::Maybe<void> V8PersistentCallbackInterface<V8EntriesCallback>::handleEvent(ScriptWrappable* callback_this_value, const HeapVector<Member<Entry>>& entries) {
  return Proxy()->handleEvent(
      callback_this_value, entries);
}

MODULES_EXTERN_TEMPLATE_EXPORT
void V8PersistentCallbackInterface<V8EntriesCallback>::InvokeAndReportException(ScriptWrappable* callback_this_value, const HeapVector<Member<Entry>>& entries) {
