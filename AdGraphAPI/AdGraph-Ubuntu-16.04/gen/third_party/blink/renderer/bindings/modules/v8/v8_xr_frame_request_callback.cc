...
v8::Maybe<void> V8XRFrameRequestCallback::Invoke(ScriptWrappable* callback_this_value, double time, XRPresentationFrame* frame) {
  ...
  v8::Local<v8::Value> v8_time = v8::Number::New(GetIsolate(), time);
  v8::Local<v8::Value> v8_frame = ToV8(frame, argument_creation_context, GetIsolate());
  v8::Local<v8::Value> argv[] = { v8_time, v8_frame };
...
void V8XRFrameRequestCallback::InvokeAndReportException(ScriptWrappable* callback_this_value, double time, XRPresentationFrame* frame) {
  v8::TryCatch try_catch(GetIsolate());
  try_catch.SetVerbose(true);

  v8::Maybe<void> maybe_result =
      Invoke(callback_this_value, time, frame);
  // An exception if any is killed with the v8::TryCatch above.
  ALLOW_UNUSED_LOCAL(maybe_result);
}

MODULES_TEMPLATE_EXPORT
v8::Maybe<void> V8PersistentCallbackFunction<V8XRFrameRequestCallback>::Invoke(ScriptWrappable* callback_this_value, double time, XRPresentationFrame* frame) {
  return Proxy()->Invoke(
      callback_this_value, time, frame);
}

MODULES_TEMPLATE_EXPORT
void V8PersistentCallbackFunction<V8XRFrameRequestCallback>::InvokeAndReportException(ScriptWrappable* callback_this_value, double time, XRPresentationFrame* frame) {
