...
v8::Maybe<void> V8FileSystemCallback::handleEvent(ScriptWrappable* callback_this_value, DOMFileSystem* fileSystem) {
  ...
  v8::Local<v8::Value> fileSystemHandle = ToV8(fileSystem, argument_creation_context, GetIsolate());
...
void V8FileSystemCallback::InvokeAndReportException(ScriptWrappable* callback_this_value, DOMFileSystem* fileSystem) {
  v8::TryCatch try_catch(GetIsolate());
  try_catch.SetVerbose(true);

  v8::Maybe<void> maybe_result =
      handleEvent(callback_this_value, fileSystem);
  // An exception if any is killed with the v8::TryCatch above.
  ALLOW_UNUSED_LOCAL(maybe_result);
}

MODULES_EXTERN_TEMPLATE_EXPORT
v8::Maybe<void> V8PersistentCallbackInterface<V8FileSystemCallback>::handleEvent(ScriptWrappable* callback_this_value, DOMFileSystem* fileSystem) {
  return Proxy()->handleEvent(
      callback_this_value, fileSystem);
}

MODULES_EXTERN_TEMPLATE_EXPORT
void V8PersistentCallbackInterface<V8FileSystemCallback>::InvokeAndReportException(ScriptWrappable* callback_this_value, DOMFileSystem* fileSystem) {
