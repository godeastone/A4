// Copyright 2014 The Chromium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// This file has been auto-generated from the Jinja2 template
// third_party/blink/renderer/bindings/templates/dictionary_v8.cpp.tmpl
// by the script code_generator_v8.py.
// DO NOT MODIFY!

// clang-format off
#include "third_party/blink/renderer/bindings/modules/v8/v8_iir_filter_options.h"

#include "third_party/blink/renderer/bindings/core/v8/exception_state.h"
#include "third_party/blink/renderer/bindings/core/v8/idl_types.h"
#include "third_party/blink/renderer/bindings/core/v8/native_value_traits_impl.h"
#include "third_party/blink/renderer/bindings/modules/v8/v8_audio_node_options.h"

namespace blink {

static const v8::Eternal<v8::Name>* eternalV8IIRFilterOptionsKeys(v8::Isolate* isolate) {
  static const char* const kKeys[] = {
    "feedback",
    "feedforward",
  };
  return V8PerIsolateData::From(isolate)->FindOrCreateEternalNameCache(
      kKeys, kKeys, arraysize(kKeys));
}

void V8IIRFilterOptions::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8Value, IIRFilterOptions& impl, ExceptionState& exceptionState) {
  if (IsUndefinedOrNull(v8Value)) {
    exceptionState.ThrowTypeError("Missing required member(s): feedback, feedforward.");
    return;
  }
  if (!v8Value->IsObject()) {
    exceptionState.ThrowTypeError("cannot convert to dictionary.");
    return;
  }
  v8::Local<v8::Object> v8Object = v8Value.As<v8::Object>();
  ALLOW_UNUSED_LOCAL(v8Object);

  V8AudioNodeOptions::ToImpl(isolate, v8Value, impl, exceptionState);
  if (exceptionState.HadException())
    return;

  const v8::Eternal<v8::Name>* keys = eternalV8IIRFilterOptionsKeys(isolate);
  v8::TryCatch block(isolate);
  v8::Local<v8::Context> context = isolate->GetCurrentContext();
  v8::Local<v8::Value> feedbackValue;
  if (!v8Object->Get(context, keys[0].Get(isolate)).ToLocal(&feedbackValue)) {
    exceptionState.RethrowV8Exception(block.Exception());
    return;
  }
  if (feedbackValue.IsEmpty() || feedbackValue->IsUndefined()) {
    exceptionState.ThrowTypeError("required member feedback is undefined.");
    return;
  } else {
    Vector<double> feedbackCppValue = NativeValueTraits<IDLSequence<IDLDouble>>::NativeValue(isolate, feedbackValue, exceptionState);
    if (exceptionState.HadException())
      return;
    impl.setFeedback(feedbackCppValue);
  }

  v8::Local<v8::Value> feedforwardValue;
  if (!v8Object->Get(context, keys[1].Get(isolate)).ToLocal(&feedforwardValue)) {
    exceptionState.RethrowV8Exception(block.Exception());
    return;
  }
  if (feedforwardValue.IsEmpty() || feedforwardValue->IsUndefined()) {
    exceptionState.ThrowTypeError("required member feedforward is undefined.");
    return;
  } else {
    Vector<double> feedforwardCppValue = NativeValueTraits<IDLSequence<IDLDouble>>::NativeValue(isolate, feedforwardValue, exceptionState);
    if (exceptionState.HadException())
      return;
    impl.setFeedforward(feedforwardCppValue);
  }
}

v8::Local<v8::Value> IIRFilterOptions::ToV8Impl(v8::Local<v8::Object> creationContext, v8::Isolate* isolate) const {
  v8::Local<v8::Object> v8Object = v8::Object::New(isolate);
  if (!toV8IIRFilterOptions(*this, v8Object, creationContext, isolate))
    return v8::Undefined(isolate);
  return v8Object;
}

bool toV8IIRFilterOptions(const IIRFilterOptions& impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  if (!toV8AudioNodeOptions(impl, dictionary, creationContext, isolate))
    return false;

  const v8::Eternal<v8::Name>* keys = eternalV8IIRFilterOptionsKeys(isolate);
  v8::Local<v8::Context> context = isolate->GetCurrentContext();
  v8::Local<v8::Value> feedbackValue;
  bool feedbackHasValueOrDefault = false;
  if (impl.hasFeedback()) {
    feedbackValue = ToV8(impl.feedback(), creationContext, isolate);
    feedbackHasValueOrDefault = true;
  } else {
    NOTREACHED();
  }
  if (feedbackHasValueOrDefault &&
      !V8CallBoolean(dictionary->CreateDataProperty(context, keys[0].Get(isolate), feedbackValue))) {
    return false;
  }

  v8::Local<v8::Value> feedforwardValue;
  bool feedforwardHasValueOrDefault = false;
  if (impl.hasFeedforward()) {
    feedforwardValue = ToV8(impl.feedforward(), creationContext, isolate);
    feedforwardHasValueOrDefault = true;
  } else {
    NOTREACHED();
  }
  if (feedforwardHasValueOrDefault &&
      !V8CallBoolean(dictionary->CreateDataProperty(context, keys[1].Get(isolate), feedforwardValue))) {
    return false;
  }

  return true;
}

IIRFilterOptions NativeValueTraits<IIRFilterOptions>::NativeValue(v8::Isolate* isolate, v8::Local<v8::Value> value, ExceptionState& exceptionState) {
  IIRFilterOptions impl;
  V8IIRFilterOptions::ToImpl(isolate, value, impl, exceptionState);
  return impl;
}

}  // namespace blink
