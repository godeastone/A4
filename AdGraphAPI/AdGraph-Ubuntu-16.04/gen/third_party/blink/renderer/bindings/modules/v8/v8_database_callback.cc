...
v8::Maybe<void> V8DatabaseCallback::Invoke(ScriptWrappable* callback_this_value, Database* database) {
  ...
  v8::Local<v8::Value> v8_database = ToV8(database, argument_creation_context, GetIsolate());
  v8::Local<v8::Value> argv[] = { v8_database };
...
void V8DatabaseCallback::InvokeAndReportException(ScriptWrappable* callback_this_value, Database* database) {
  v8::TryCatch try_catch(GetIsolate());
  try_catch.SetVerbose(true);

  v8::Maybe<void> maybe_result =
      Invoke(callback_this_value, database);
  // An exception if any is killed with the v8::TryCatch above.
  ALLOW_UNUSED_LOCAL(maybe_result);
}

MODULES_TEMPLATE_EXPORT
v8::Maybe<void> V8PersistentCallbackFunction<V8DatabaseCallback>::Invoke(ScriptWrappable* callback_this_value, Database* database) {
  return Proxy()->Invoke(
      callback_this_value, database);
}

MODULES_TEMPLATE_EXPORT
void V8PersistentCallbackFunction<V8DatabaseCallback>::InvokeAndReportException(ScriptWrappable* callback_this_value, Database* database) {
