...
void V8FocusEventInit::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8Value, FocusEventInit& impl, ExceptionState& exceptionState) {
  ...
  if (!v8Object->Get(context, keys[0].Get(isolate)).ToLocal(&relatedTargetValue)) {
...
bool toV8FocusEventInit(const FocusEventInit& impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  ...
  if (relatedTargetHasValueOrDefault &&
      !V8CallBoolean(dictionary->CreateDataProperty(context, keys[0].Get(isolate), relatedTargetValue))) {
    return false;
  }
