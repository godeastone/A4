...
void V8InputDeviceCapabilitiesInit::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8Value, InputDeviceCapabilitiesInit& impl, ExceptionState& exceptionState) {
  ...
  if (!v8Object->Get(context, keys[0].Get(isolate)).ToLocal(&firesTouchEventsValue)) {
  ...
  if (firesTouchEventsValue.IsEmpty() || firesTouchEventsValue->IsUndefined()) {
    ...
    bool firesTouchEventsCppValue = NativeValueTraits<IDLBoolean>::NativeValue(isolate, firesTouchEventsValue, exceptionState);
    if (exceptionState.HadException())
      return;
    impl.setFiresTouchEvents(firesTouchEventsCppValue);
  }
}
...
bool toV8InputDeviceCapabilitiesInit(const InputDeviceCapabilitiesInit& impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  ...
  v8::Local<v8::Context> context = isolate->GetCurrentContext();
  v8::Local<v8::Value> firesTouchEventsValue;
  bool firesTouchEventsHasValueOrDefault = false;
  if (impl.hasFiresTouchEvents()) {
    firesTouchEventsValue = v8::Boolean::New(isolate, impl.firesTouchEvents());
    firesTouchEventsHasValueOrDefault = true;
  } else {
    firesTouchEventsValue = v8::Boolean::New(isolate, false);
    firesTouchEventsHasValueOrDefault = true;
  }
  if (firesTouchEventsHasValueOrDefault &&
      !V8CallBoolean(dictionary->CreateDataProperty(context, keys[0].Get(isolate), firesTouchEventsValue))) {
    return false;
  }
