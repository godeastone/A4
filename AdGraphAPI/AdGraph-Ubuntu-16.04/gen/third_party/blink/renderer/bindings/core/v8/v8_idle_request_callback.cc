...
v8::Maybe<void> V8IdleRequestCallback::Invoke(ScriptWrappable* callback_this_value, IdleDeadline* deadline) {
  ...
  v8::Local<v8::Value> v8_deadline = ToV8(deadline, argument_creation_context, GetIsolate());
  v8::Local<v8::Value> argv[] = { v8_deadline };
...
void V8IdleRequestCallback::InvokeAndReportException(ScriptWrappable* callback_this_value, IdleDeadline* deadline) {
  v8::TryCatch try_catch(GetIsolate());
  try_catch.SetVerbose(true);

  v8::Maybe<void> maybe_result =
      Invoke(callback_this_value, deadline);
  // An exception if any is killed with the v8::TryCatch above.
  ALLOW_UNUSED_LOCAL(maybe_result);
}

CORE_TEMPLATE_EXPORT
v8::Maybe<void> V8PersistentCallbackFunction<V8IdleRequestCallback>::Invoke(ScriptWrappable* callback_this_value, IdleDeadline* deadline) {
  return Proxy()->Invoke(
      callback_this_value, deadline);
}

CORE_TEMPLATE_EXPORT
void V8PersistentCallbackFunction<V8IdleRequestCallback>::InvokeAndReportException(ScriptWrappable* callback_this_value, IdleDeadline* deadline) {
