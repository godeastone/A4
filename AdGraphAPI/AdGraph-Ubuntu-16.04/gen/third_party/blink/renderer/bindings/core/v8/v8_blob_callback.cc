...
v8::Maybe<void> V8BlobCallback::Invoke(ScriptWrappable* callback_this_value, Blob* blob) {
  ...
  v8::Local<v8::Value> v8_blob = ToV8(blob, argument_creation_context, GetIsolate());
  v8::Local<v8::Value> argv[] = { v8_blob };
...
void V8BlobCallback::InvokeAndReportException(ScriptWrappable* callback_this_value, Blob* blob) {
  v8::TryCatch try_catch(GetIsolate());
  try_catch.SetVerbose(true);

  v8::Maybe<void> maybe_result =
      Invoke(callback_this_value, blob);
  // An exception if any is killed with the v8::TryCatch above.
  ALLOW_UNUSED_LOCAL(maybe_result);
}

CORE_TEMPLATE_EXPORT
v8::Maybe<void> V8PersistentCallbackFunction<V8BlobCallback>::Invoke(ScriptWrappable* callback_this_value, Blob* blob) {
  return Proxy()->Invoke(
      callback_this_value, blob);
}

CORE_TEMPLATE_EXPORT
void V8PersistentCallbackFunction<V8BlobCallback>::InvokeAndReportException(ScriptWrappable* callback_this_value, Blob* blob) {
