...
void V8AssignedNodesOptions::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8Value, AssignedNodesOptions& impl, ExceptionState& exceptionState) {
  ...
  if (!v8Object->Get(context, keys[0].Get(isolate)).ToLocal(&flattenValue)) {
  ...
  if (flattenValue.IsEmpty() || flattenValue->IsUndefined()) {
    ...
    bool flattenCppValue = NativeValueTraits<IDLBoolean>::NativeValue(isolate, flattenValue, exceptionState);
    if (exceptionState.HadException())
      return;
    impl.setFlatten(flattenCppValue);
  }
}
...
bool toV8AssignedNodesOptions(const AssignedNodesOptions& impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  ...
  v8::Local<v8::Context> context = isolate->GetCurrentContext();
  v8::Local<v8::Value> flattenValue;
  bool flattenHasValueOrDefault = false;
  if (impl.hasFlatten()) {
    flattenValue = v8::Boolean::New(isolate, impl.flatten());
    flattenHasValueOrDefault = true;
  } else {
    flattenValue = v8::Boolean::New(isolate, false);
    flattenHasValueOrDefault = true;
  }
  if (flattenHasValueOrDefault &&
      !V8CallBoolean(dictionary->CreateDataProperty(context, keys[0].Get(isolate), flattenValue))) {
    return false;
  }
