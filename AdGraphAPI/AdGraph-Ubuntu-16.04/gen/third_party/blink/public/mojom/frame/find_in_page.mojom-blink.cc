...
void FindInPageProxy::ActivateNearestFindResult(
    const ::blink::WebFloatPoint& in_point, ActivateNearestFindResultCallback callback) {
#if BUILDFLAG(MOJO_TRACE_ENABLED)
...
class FindInPage_ActivateNearestFindResult_ProxyToResponder {
 ...
 private:
  ...
  void Run(
      const ::blink::WebRect& in_active_match_rect, int32_t in_number_of_matches, int32_t in_active_match_ordinal, bool in_final_update);
...
void FindInPageInterceptorForTesting::GetNearestFindResult(const ::blink::WebFloatPoint& point, GetNearestFindResultCallback callback) {
  GetForwardingInterface()->GetNearestFindResult(std::move(point), std::move(callback));
}
void FindInPageInterceptorForTesting::ActivateNearestFindResult(const ::blink::WebFloatPoint& point, ActivateNearestFindResultCallback callback) {
...
void FindInPageAsyncWaiter::ActivateNearestFindResult(
  ...
  proxy_->ActivateNearestFindResult(std::move(point),
      base::BindOnce(
          [](base::RunLoop* loop,
             ::blink::WebRect* out_active_match_rect
,
...
,
             const ::blink::WebRect& active_match_rect,
  ...
  proxy_->FindMatchRects(std::move(current_version),
      base::BindOnce(
          [](base::RunLoop* loop,
             int32_t* out_version
,
             WTF::Vector<::blink::WebFloatRect>* out_rects
,
             ::blink::WebFloatRect* out_active_match_rect
,
             int32_t version,
             const WTF::Vector<::blink::WebFloatRect>& rects,
             const ::blink::WebFloatRect& active_match_rect) {*out_version = std::move(version);*out_rects = std::move(rects);*out_active_match_rect = std::move(active_match_rect);
            loop->Quit();
          },
          &loop,
          out_version,
