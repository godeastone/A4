...
class Blob_ReadSideData_ProxyToResponder {
 ...
 private:
  ...
  void Run(
      const base::Optional<std::vector<uint8_t>>& in_data);
...
void BlobInterceptorForTesting::AsDataPipeGetter(::network::mojom::DataPipeGetterRequest data_pipe_getter) {
  ...
  proxy_->ReadSideData(
      base::BindOnce(
          [](base::RunLoop* loop,
             base::Optional<std::vector<uint8_t>>* out_data
,
             const base::Optional<std::vector<uint8_t>>& data) {*out_data = std::move(data);
            loop->Quit();
          },
          &loop,
          out_data));
