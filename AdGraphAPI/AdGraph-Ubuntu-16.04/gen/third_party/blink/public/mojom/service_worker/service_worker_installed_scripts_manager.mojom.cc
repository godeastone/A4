...
ServiceWorkerInstalledScriptsInfo::ServiceWorkerInstalledScriptsInfo(
    ...
    const std::vector<GURL>& installed_urls_in)
...
void ServiceWorkerInstalledScriptsManagerHostProxy::RequestInstalledScript(
  ...
  TRACE_EVENT0("mojom", "blink::mojom::ServiceWorkerInstalledScriptsManagerHost::RequestInstalledScript");
#endif
...
#if BUILDFLAG(MOJO_TRACE_ENABLED)
  TRACE_EVENT1("mojom", "(Impl)blink::mojom::ServiceWorkerInstalledScriptsManagerHost::RequestInstalledScript",
      ...
      if (!success) {
        ReportValidationErrorForMessage(
            ...
            "ServiceWorkerInstalledScriptsManagerHost::RequestInstalledScript deserializer");
        return false;
      }
...
#if BUILDFLAG(MOJO_TRACE_ENABLED)
  TRACE_EVENT0("mojom", "blink::mojom::ServiceWorkerInstalledScriptsManager::TransferInstalledScript");
#endif
  const bool kExpectsResponse = false;
  const bool kIsSync = false;
...
#if BUILDFLAG(MOJO_TRACE_ENABLED)
  TRACE_EVENT1("mojom", "(Impl)blink::mojom::ServiceWorkerInstalledScriptsManager::TransferInstalledScript",
               "message", message->name());
#endif
      mojo::internal::MessageDispatchContext context(message);
      ...
      if (!success) {
        ReportValidationErrorForMessage(
            message,
            mojo::internal::VALIDATION_ERROR_DESERIALIZATION_FAILED,
            "ServiceWorkerInstalledScriptsManager::TransferInstalledScript deserializer");
        return false;
      }
      // A null |impl| means no implementation was bound.
      assert(impl);
