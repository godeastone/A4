  ...
  proxy_->GetRegistrations(std::move(service_worker_registration_id),
      base::BindOnce(
          [](base::RunLoop* loop,
             BackgroundSyncError* out_err
,
             std::vector<SyncRegistrationPtr>* out_registrations
,
             BackgroundSyncError err,
             std::vector<SyncRegistrationPtr> registrations) {*out_err = std::move(err);*out_registrations = std::move(registrations);
            loop->Quit();
          },
          &loop,
          out_err,
