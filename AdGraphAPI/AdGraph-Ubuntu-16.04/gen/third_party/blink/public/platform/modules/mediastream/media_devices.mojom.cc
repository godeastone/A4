...
class MediaDevicesDispatcherHost_EnumerateDevices_ProxyToResponder {
 ...
 private:
  ...
  void Run(
      const std::vector<std::vector<content::MediaDeviceInfo>>& in_enumeration, std::vector<VideoInputDeviceCapabilitiesPtr> in_video_input_device_capabilities);
...
void MediaDevicesDispatcherHostAsyncWaiter::EnumerateDevices(
  ...
  proxy_->EnumerateDevices(std::move(request_audio_input),std::move(request_video_input),std::move(request_audio_output),std::move(request_video_input_capabilities),
      base::BindOnce(
          [](base::RunLoop* loop,
             std::vector<std::vector<content::MediaDeviceInfo>>* out_enumeration
,
             std::vector<VideoInputDeviceCapabilitiesPtr>* out_video_input_device_capabilities
,
             const std::vector<std::vector<content::MediaDeviceInfo>>& enumeration,
             std::vector<VideoInputDeviceCapabilitiesPtr> video_input_device_capabilities) {*out_enumeration = std::move(enumeration);*out_video_input_device_capabilities = std::move(video_input_device_capabilities);
            loop->Quit();
          },
          &loop,
          out_enumeration,
...
void MediaDevicesListenerProxy::OnDevicesChanged(
    content::MediaDeviceType in_type, const std::vector<content::MediaDeviceInfo>& in_device_infos) {
#if BUILDFLAG(MOJO_TRACE_ENABLED)
...
void MediaDevicesListenerInterceptorForTesting::OnDevicesChanged(content::MediaDeviceType type, const std::vector<content::MediaDeviceInfo>& device_infos) {
