...
std::unique_ptr<GetBackgroundColorsResult> GetBackgroundColorsResult::Parse(const base::Value& value, ErrorReporter* errors) {
  ...
  const base::Value* computed_body_font_size_value = value.FindKey("computedBodyFontSize");
  if (computed_body_font_size_value) {
    errors->SetName("computedBodyFontSize");
    result->computed_body_font_size_ = internal::FromValue<std::string>::Parse(*computed_body_font_size_value, errors);
  }
  errors->Pop();
  errors->Pop();
  if (errors->HasErrors())
...
std::unique_ptr<base::Value> GetBackgroundColorsResult::Serialize() const {
  ...
  if (computed_body_font_size_)
    result->Set("computedBodyFontSize", internal::ToValue(computed_body_font_size_.value()));
  return std::move(result);
}
