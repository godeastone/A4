...
std::unique_ptr<CustomPreview> CustomPreview::Parse(const base::Value& value, ErrorReporter* errors) {
  ...
  const base::Value* has_body_value = value.FindKey("hasBody");
  if (has_body_value) {
  ...
  const base::Value* formatter_object_id_value = value.FindKey("formatterObjectId");
  if (formatter_object_id_value) {
  ...
  const base::Value* bind_remote_object_function_id_value = value.FindKey("bindRemoteObjectFunctionId");
  if (bind_remote_object_function_id_value) {
    errors->SetName("bindRemoteObjectFunctionId");
    result->bind_remote_object_function_id_ = internal::FromValue<std::string>::Parse(*bind_remote_object_function_id_value, errors);
  } else {
    errors->AddError("required property missing: bindRemoteObjectFunctionId");
  }
  const base::Value* config_object_id_value = value.FindKey("configObjectId");
  if (config_object_id_value) {
    errors->SetName("configObjectId");
    result->config_object_id_ = internal::FromValue<std::string>::Parse(*config_object_id_value, errors);
  }
  errors->Pop();
  errors->Pop();
  if (errors->HasErrors())
...
std::unique_ptr<base::Value> CustomPreview::Serialize() const {
  ...
  result->Set("bindRemoteObjectFunctionId", internal::ToValue(bind_remote_object_function_id_));
  if (config_object_id_)
    result->Set("configObjectId", internal::ToValue(config_object_id_.value()));
  return std::move(result);
}
