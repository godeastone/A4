...
std::unique_ptr<RequestAppBannerParams> RequestAppBannerParams::Parse(const base::Value& value, ErrorReporter* errors) {
  errors->Push();
  errors->SetName("RequestAppBannerParams");
  if (!value.is_dict()) {
    errors->AddError("object expected");
    errors->Pop();
    return nullptr;
  ...
  std::unique_ptr<RequestAppBannerParams> result(new RequestAppBannerParams());
  errors->Push();
  errors->SetName("RequestAppBannerParams");
  errors->Pop();
  errors->Pop();
...
std::unique_ptr<base::Value> RequestAppBannerParams::Serialize() const {
  std::unique_ptr<base::DictionaryValue> result(new base::DictionaryValue());
  return std::move(result);
}

std::unique_ptr<RequestAppBannerParams> RequestAppBannerParams::Clone() const {
  ErrorReporter errors;
  std::unique_ptr<RequestAppBannerParams> result = Parse(*Serialize(), &errors);
  DCHECK(!errors.HasErrors());
  return result;
}


std::unique_ptr<RequestAppBannerResult> RequestAppBannerResult::Parse(const base::Value& value, ErrorReporter* errors) {
  errors->Push();
  errors->SetName("RequestAppBannerResult");
  if (!value.is_dict()) {
    errors->AddError("object expected");
    errors->Pop();
    return nullptr;
  ...
  std::unique_ptr<RequestAppBannerResult> result(new RequestAppBannerResult());
  errors->Push();
  errors->SetName("RequestAppBannerResult");
  errors->Pop();
  errors->Pop();
...
std::unique_ptr<base::Value> RequestAppBannerResult::Serialize() const {
  std::unique_ptr<base::DictionaryValue> result(new base::DictionaryValue());
  return std::move(result);
}

std::unique_ptr<RequestAppBannerResult> RequestAppBannerResult::Clone() const {
  ErrorReporter errors;
  std::unique_ptr<RequestAppBannerResult> result = Parse(*Serialize(), &errors);
  DCHECK(!errors.HasErrors());
  return result;
}
...
std::unique_ptr<FrameScheduledNavigationParams> FrameScheduledNavigationParams::Parse(const base::Value& value, ErrorReporter* errors) {
  ...
  if (reason_value) {
    ...
    result->reason_ = internal::FromValue<::headless::page::FrameScheduledNavigationReason>::Parse(*reason_value, errors);
  } else {
