...
void ExperimentalDomain::Disable(std::unique_ptr<DisableParams> params, base::OnceCallback<void(std::unique_ptr<DisableResult>)> callback) {
  dispatcher_->SendMessage("Database.disable", params->Serialize(), base::BindOnce(&Domain::HandleDisableResponse, std::move(callback)));
}
...
void ExperimentalDomain::Enable(std::unique_ptr<EnableParams> params, base::OnceCallback<void(std::unique_ptr<EnableResult>)> callback) {
  dispatcher_->SendMessage("Database.enable", params->Serialize(), base::BindOnce(&Domain::HandleEnableResponse, std::move(callback)));
}

void ExperimentalDomain::ExecuteSQL(std::unique_ptr<ExecuteSQLParams> params, base::OnceCallback<void(std::unique_ptr<ExecuteSQLResult>)> callback) {
  dispatcher_->SendMessage("Database.executeSQL", params->Serialize(), base::BindOnce(&Domain::HandleExecuteSQLResponse, std::move(callback)));
}

void ExperimentalDomain::GetDatabaseTableNames(std::unique_ptr<GetDatabaseTableNamesParams> params, base::OnceCallback<void(std::unique_ptr<GetDatabaseTableNamesResult>)> callback) {
  dispatcher_->SendMessage("Database.getDatabaseTableNames", params->Serialize(), base::BindOnce(&Domain::HandleGetDatabaseTableNamesResponse, std::move(callback)));
}
...
// static
void Domain::HandleExecuteSQLResponse(base::OnceCallback<void(std::unique_ptr<ExecuteSQLResult>)> callback, const base::Value& response) {
  if (callback.is_null())
    return;
  // This is an error response.
  if (response.is_none()) {
...
// static
void Domain::HandleGetDatabaseTableNamesResponse(base::OnceCallback<void(std::unique_ptr<GetDatabaseTableNamesResult>)> callback, const base::Value& response) {
  if (callback.is_null())
    return;
  // This is an error response.
  if (response.is_none()) {
