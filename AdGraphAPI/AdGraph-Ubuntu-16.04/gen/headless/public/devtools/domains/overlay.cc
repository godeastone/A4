...
void ExperimentalDomain::SetSuspended(std::unique_ptr<SetSuspendedParams> params, base::OnceCallback<void(std::unique_ptr<SetSuspendedResult>)> callback) {
  dispatcher_->SendMessage("Overlay.setSuspended", params->Serialize(), base::BindOnce(&Domain::HandleSetSuspendedResponse, std::move(callback)));
}
...
// static
void Domain::HandleSetSuspendedResponse(base::OnceCallback<void(std::unique_ptr<SetSuspendedResult>)> callback, const base::Value& response) {
  if (callback.is_null())
    return;
  // This is an error response.
  if (response.is_none()) {
  ...
  ErrorReporter errors;
  std::unique_ptr<SetSuspendedResult> result = SetSuspendedResult::Parse(response, &errors);
  DCHECK(!errors.HasErrors()) << errors.ToString();
  std::move(callback).Run(std::move(result));
}
