...
void ExperimentalDomain::ScheduleStepIntoAsync(std::unique_ptr<ScheduleStepIntoAsyncParams> params, base::OnceCallback<void(std::unique_ptr<ScheduleStepIntoAsyncResult>)> callback) {
  dispatcher_->SendMessage("Debugger.scheduleStepIntoAsync", params->Serialize(), base::BindOnce(&Domain::HandleScheduleStepIntoAsyncResponse, std::move(callback)));
}
...
// static
void Domain::HandleScheduleStepIntoAsyncResponse(base::OnceCallback<void(std::unique_ptr<ScheduleStepIntoAsyncResult>)> callback, const base::Value& response) {
  if (callback.is_null())
    return;
  // This is an error response.
  if (response.is_none()) {
  ...
  ErrorReporter errors;
  std::unique_ptr<ScheduleStepIntoAsyncResult> result = ScheduleStepIntoAsyncResult::Parse(response, &errors);
  DCHECK(!errors.HasErrors()) << errors.ToString();
  std::move(callback).Run(std::move(result));
}
