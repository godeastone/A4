...
class HEADLESS_EXPORT ScheduleStepIntoAsyncParams {
 public:
  static std::unique_ptr<ScheduleStepIntoAsyncParams> Parse(const base::Value& value, ErrorReporter* errors);
  ~ScheduleStepIntoAsyncParams() { }

  std::unique_ptr<base::Value> Serialize() const;
  std::unique_ptr<ScheduleStepIntoAsyncParams> Clone() const;

  template<int STATE>
  class ScheduleStepIntoAsyncParamsBuilder {
  public:
    enum {
      kNoFieldsSet = 0,
      kAllRequiredFieldsSet = (0)
    ...
    std::unique_ptr<ScheduleStepIntoAsyncParams> Build() {
      static_assert(STATE == kAllRequiredFieldsSet, "all required fields should have been set");
      return std::move(result_);
    }

   private:
    friend class ScheduleStepIntoAsyncParams;
    ScheduleStepIntoAsyncParamsBuilder() : result_(new ScheduleStepIntoAsyncParams()) { }

    template<int STEP> ScheduleStepIntoAsyncParamsBuilder<STATE | STEP>& CastState() {
      return *reinterpret_cast<ScheduleStepIntoAsyncParamsBuilder<STATE | STEP>*>(this);
    }

    std::unique_ptr<ScheduleStepIntoAsyncParams> result_;
  };

  static ScheduleStepIntoAsyncParamsBuilder<0> Builder() {
    return ScheduleStepIntoAsyncParamsBuilder<0>();
  }
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(ScheduleStepIntoAsyncParams);
};


// Result for the ScheduleStepIntoAsync command.
class HEADLESS_EXPORT ScheduleStepIntoAsyncResult {
 public:
  static std::unique_ptr<ScheduleStepIntoAsyncResult> Parse(const base::Value& value, ErrorReporter* errors);
  ~ScheduleStepIntoAsyncResult() { }

  std::unique_ptr<base::Value> Serialize() const;
  std::unique_ptr<ScheduleStepIntoAsyncResult> Clone() const;

  template<int STATE>
  class ScheduleStepIntoAsyncResultBuilder {
  public:
    enum {
      kNoFieldsSet = 0,
      kAllRequiredFieldsSet = (0)
    ...
    std::unique_ptr<ScheduleStepIntoAsyncResult> Build() {
      static_assert(STATE == kAllRequiredFieldsSet, "all required fields should have been set");
      return std::move(result_);
    }

   private:
    friend class ScheduleStepIntoAsyncResult;
    ScheduleStepIntoAsyncResultBuilder() : result_(new ScheduleStepIntoAsyncResult()) { }

    template<int STEP> ScheduleStepIntoAsyncResultBuilder<STATE | STEP>& CastState() {
      return *reinterpret_cast<ScheduleStepIntoAsyncResultBuilder<STATE | STEP>*>(this);
    }

    std::unique_ptr<ScheduleStepIntoAsyncResult> result_;
  };

  static ScheduleStepIntoAsyncResultBuilder<0> Builder() {
    return ScheduleStepIntoAsyncResultBuilder<0>();
  }
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(ScheduleStepIntoAsyncResult);
};
