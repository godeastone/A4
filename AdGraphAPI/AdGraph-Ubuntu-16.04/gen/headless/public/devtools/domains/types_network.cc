...
std::unique_ptr<RequestPattern> RequestPattern::Parse(const base::Value& value, ErrorReporter* errors) {
  ...
  if (resource_type_value) {
    ...
    result->resource_type_ = internal::FromValue<::headless::page::ResourceType>::Parse(*resource_type_value, errors);
  }
...
std::unique_ptr<SignedExchangeSignature> SignedExchangeSignature::Parse(const base::Value& value, ErrorReporter* errors) {
  ...
  if (cert_url_value) {
    ...
    errors->AddError("required property missing: certUrl");
  }
...
std::unique_ptr<base::Value> SignedExchangeSignature::Serialize() const {
  ...
  result->Set("certUrl", internal::ToValue(cert_url_));
...
std::unique_ptr<SignedExchangeInfo> SignedExchangeInfo::Parse(const base::Value& value, ErrorReporter* errors) {
  ...
  if (errors_value) {
    ...
    result->errors_ = internal::FromValue<std::vector<std::string>>::Parse(*errors_value, errors);
  }
  errors->Pop();
  errors->Pop();
  if (errors->HasErrors())
...
std::unique_ptr<ContinueInterceptedRequestParams> ContinueInterceptedRequestParams::Parse(const base::Value& value, ErrorReporter* errors) {
  ...
  if (raw_response_value) {
    ...
    result->raw_response_ = internal::FromValue<std::string>::Parse(*raw_response_value, errors);
  }
  const base::Value* url_value = value.FindKey("url");
  if (url_value) {
    errors->SetName("url");
...
std::unique_ptr<LoadingFinishedParams> LoadingFinishedParams::Parse(const base::Value& value, ErrorReporter* errors) {
  ...
  const base::Value* blocked_cross_site_document_value = value.FindKey("blockedCrossSiteDocument");
  if (blocked_cross_site_document_value) {
    errors->SetName("blockedCrossSiteDocument");
    result->blocked_cross_site_document_ = internal::FromValue<bool>::Parse(*blocked_cross_site_document_value, errors);
  }
  errors->Pop();
  errors->Pop();
  if (errors->HasErrors())
...
std::unique_ptr<base::Value> LoadingFinishedParams::Serialize() const {
  ...
  result->Set("timestamp", internal::ToValue(timestamp_));
  result->Set("encodedDataLength", internal::ToValue(encoded_data_length_));
  if (blocked_cross_site_document_)
    result->Set("blockedCrossSiteDocument", internal::ToValue(blocked_cross_site_document_.value()));
  return std::move(result);
}
