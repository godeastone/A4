...
class JpegEncodeAccelerator_EncodeWithFD_ProxyToResponder {
 ...
 private:
  ...
  void Run(
      int32_t in_buffer_id, uint32_t in_encoded_buffer_size, media::JpegEncodeAccelerator::Status in_status);
...
bool JpegEncodeAccelerator_EncodeWithFD_ForwardToCallback::Accept(
  ...
  media::JpegEncodeAccelerator::Status p_status{};
...
,
             media::JpegEncodeAccelerator::Status* out_status
,
             int32_t buffer_id,
             uint32_t encoded_buffer_size,
             media::JpegEncodeAccelerator::Status status) {*out_buffer_id = std::move(buffer_id);*out_encoded_buffer_size = std::move(encoded_buffer_size);*out_status = std::move(status);
            loop->Quit();
          },
          &loop,
          out_buffer_id,
