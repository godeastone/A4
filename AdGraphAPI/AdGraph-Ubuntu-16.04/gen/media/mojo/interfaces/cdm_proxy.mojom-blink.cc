...
void CdmProxyProxy::SetKey(
    uint32_t in_crypto_session_id, const WTF::Vector<uint8_t>& in_key_id, const WTF::Vector<uint8_t>& in_key_blob) {
#if BUILDFLAG(MOJO_TRACE_ENABLED)
...
void CdmProxyProxy::RemoveKey(
    uint32_t in_crypto_session_id, const WTF::Vector<uint8_t>& in_key_id) {
#if BUILDFLAG(MOJO_TRACE_ENABLED)
...
void CdmProxyInterceptorForTesting::SetKey(uint32_t crypto_session_id, const WTF::Vector<uint8_t>& key_id, const WTF::Vector<uint8_t>& key_blob) {
  GetForwardingInterface()->SetKey(std::move(crypto_session_id), std::move(key_id), std::move(key_blob));
}
void CdmProxyInterceptorForTesting::RemoveKey(uint32_t crypto_session_id, const WTF::Vector<uint8_t>& key_id) {
