...
bool DispatchFetchEventParams_Data::Validate(
  ...
  if (!mojo::internal::ValidatePointerNonNullable(
          object->request_body_blob_ptrs, 2, validation_context)) {
    return false;
  }
  const mojo::internal::ContainerValidateParams request_body_blob_ptrs_validate_params(
      0, false, nullptr);
  if (!mojo::internal::ValidateContainer(object->request_body_blob_ptrs, validation_context,
                                         &request_body_blob_ptrs_validate_params)) {
    return false;
  }

  if (!mojo::internal::ValidatePointerNonNullable(
          object->request_body_blob_uuid, 3, validation_context)) {
    return false;
  }
  const mojo::internal::ContainerValidateParams request_body_blob_uuid_validate_params(
      0, false, nullptr);
  if (!mojo::internal::ValidateContainer(object->request_body_blob_uuid, validation_context,
                                         &request_body_blob_uuid_validate_params)) {
    return false;
  }

  if (!mojo::internal::ValidateHandleOrInterface(object->request_body_blob,
                                                 validation_context)) {
    return false;
  }

  if (!mojo::internal::ValidatePointerNonNullable(
          object->client_id, 6, validation_context)) {
    return false;
  }
