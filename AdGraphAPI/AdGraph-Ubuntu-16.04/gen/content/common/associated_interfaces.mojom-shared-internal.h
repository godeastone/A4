...
#pragma pack(push, 1)
class COMPONENT_EXPORT(MOJOM_SHARED_CONTENT_EXPORT) AssociatedInterfaceProvider_GetAssociatedInterface_Params_Data {
 public:
  class BufferWriter {
   public:
 ...
 public:
  class BufferWriter {
   public:
    ...
    void Allocate(mojo::internal::Buffer* serialization_buffer) {
      serialization_buffer_ = serialization_buffer;
      index_ = serialization_buffer_->Allocate(sizeof(RouteProvider_GetRoute_Params_Data));
      new (data()) RouteProvider_GetRoute_Params_Data();
    }

    bool is_null() const { return !serialization_buffer_; }
    RouteProvider_GetRoute_Params_Data* data() {
      DCHECK(!is_null());
      return serialization_buffer_->Get<RouteProvider_GetRoute_Params_Data>(index_);
    }
    RouteProvider_GetRoute_Params_Data* operator->() { return data(); }
