...
// static
bool SpeechRecognitionHypothesis_Data::Validate(
    const void* data,
  ...
  const SpeechRecognitionHypothesis_Data* object = static_cast<const SpeechRecognitionHypothesis_Data*>(data);
...
bool SpeechRecognitionResult_Data::Validate(
  ...
  if (!mojo::internal::ValidatePointerNonNullable(
          object->hypotheses, 1, validation_context)) {
    return false;
  }
  const mojo::internal::ContainerValidateParams hypotheses_validate_params(
      0, false, nullptr);
  if (!mojo::internal::ValidateContainer(object->hypotheses, validation_context,
