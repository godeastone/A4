  ...
  class BufferWriter {
   public:
    ...
    void Allocate(mojo::internal::Buffer* serialization_buffer) {
      ...
      index_ = serialization_buffer_->Allocate(sizeof(Transform_Data));
      new (data()) Transform_Data();
    }

    bool is_null() const { return !serialization_buffer_; }
    Transform_Data* data() {
      DCHECK(!is_null());
      return serialization_buffer_->Get<Transform_Data>(index_);
    }
    Transform_Data* operator->() { return data(); }
...
struct Transform_UnserializedMessageContext
 ...
 private:
  ...
  void Serialize(mojo::internal::SerializationContext* context,
    ...
    Transform_Data::BufferWriter writer;
