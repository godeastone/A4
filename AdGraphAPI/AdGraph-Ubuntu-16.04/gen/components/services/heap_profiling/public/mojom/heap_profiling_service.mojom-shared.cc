...
bool ProfilingService_AddProfilingClient_Params_Data::Validate(
  ...
  if (!mojo::internal::ValidateHandleOrInterfaceNonNullable(
          object->memlog_pipe_receiver, 3, validation_context)) {
    return false;
  }
  if (!mojo::internal::ValidateHandleOrInterface(object->memlog_pipe_receiver,
                                                 validation_context)) {
    return false;
  }
...
// static
bool ProfilingService_SetKeepSmallAllocations_Params_Data::Validate(
    const void* data,
    mojo::internal::ValidationContext* validation_context) {
  if (!data)
    return true;
  ...
  // NOTE: The memory backing |object| may be smaller than |sizeof(*object)| if
  // the message comes from an older version.
  const ProfilingService_SetKeepSmallAllocations_Params_Data* object = static_cast<const ProfilingService_SetKeepSmallAllocations_Params_Data*>(data);
...
ProfilingService_SetKeepSmallAllocations_Params_Data::ProfilingService_SetKeepSmallAllocations_Params_Data()
    : header_({sizeof(*this), 0}) {}
