...
std::unique_ptr<base::DictionaryValue> KeyEvent::ToValue() const {
  ...
  value->SetWithoutPathExpansion("command", std::make_unique<base::Value>(braille_display_private::ToString(this->command)));

  if (this->display_position.get()) {
    value->SetWithoutPathExpansion("displayPosition", std::make_unique<base::Value>(*this->display_position));

  }
  if (this->braille_dots.get()) {
    value->SetWithoutPathExpansion("brailleDots", std::make_unique<base::Value>(*this->braille_dots));

  }
  if (this->standard_key_code.get()) {
    value->SetWithoutPathExpansion("standardKeyCode", std::make_unique<base::Value>(*this->standard_key_code));

  }
  if (this->standard_key_char.get()) {
    value->SetWithoutPathExpansion("standardKeyChar", std::make_unique<base::Value>(*this->standard_key_char));

  }
  if (this->space_key.get()) {
    value->SetWithoutPathExpansion("spaceKey", std::make_unique<base::Value>(*this->space_key));
...
std::unique_ptr<base::DictionaryValue> DisplayState::ToValue() const {
  ...
  if (this->text_row_count.get()) {
    value->SetWithoutPathExpansion("textRowCount", std::make_unique<base::Value>(*this->text_row_count));

  }
  if (this->text_column_count.get()) {
    value->SetWithoutPathExpansion("textColumnCount", std::make_unique<base::Value>(*this->text_column_count));
