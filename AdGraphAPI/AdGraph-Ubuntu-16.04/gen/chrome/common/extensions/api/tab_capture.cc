...
std::unique_ptr<base::DictionaryValue> CaptureInfo::ToValue() const {
  ...
  value->SetWithoutPathExpansion("status", std::make_unique<base::Value>(tab_capture::ToString(this->status)));

  value->SetWithoutPathExpansion("fullscreen", std::make_unique<base::Value>(this->fullscreen));
...
std::unique_ptr<base::DictionaryValue> MediaStreamConstraint::ToValue() const {
  ...
  value->SetWithoutPathExpansion("mandatory", (this->mandatory).ToValue());

  if (this->optional.get()) {
    value->SetWithoutPathExpansion("optional", (this->optional)->ToValue());
...
std::unique_ptr<base::DictionaryValue> CaptureOptions::ToValue() const {
  ...
  if (this->audio.get()) {
    value->SetWithoutPathExpansion("audio", std::make_unique<base::Value>(*this->audio));

  }
  if (this->video.get()) {
    value->SetWithoutPathExpansion("video", std::make_unique<base::Value>(*this->video));

  }
  if (this->audio_constraints.get()) {
    value->SetWithoutPathExpansion("audioConstraints", (this->audio_constraints)->ToValue());

  }
  if (this->video_constraints.get()) {
    value->SetWithoutPathExpansion("videoConstraints", (this->video_constraints)->ToValue());

  }
  if (this->presentation_id.get()) {
    value->SetWithoutPathExpansion("presentationId", std::make_unique<base::Value>(*this->presentation_id));
