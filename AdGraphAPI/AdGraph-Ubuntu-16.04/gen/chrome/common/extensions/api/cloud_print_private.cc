...
std::unique_ptr<base::DictionaryValue> PrinterSettings::ToValue() const {
  ...
  value->SetWithoutPathExpansion("connect", std::make_unique<base::Value>(this->connect));
...
std::unique_ptr<base::DictionaryValue> UserSettings::ToValue() const {
  ...
  value->SetWithoutPathExpansion("printers", json_schema_compiler::util::CreateValueFromArray(this->printers));

  value->SetWithoutPathExpansion("connectNewPrinters", std::make_unique<base::Value>(this->connect_new_printers));
