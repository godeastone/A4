...
std::unique_ptr<base::DictionaryValue> HistoryItem::ToValue() const {
  ...
  if (this->last_visit_time.get()) {
    value->SetWithoutPathExpansion("lastVisitTime", std::make_unique<base::Value>(*this->last_visit_time));

  }
  if (this->visit_count.get()) {
    value->SetWithoutPathExpansion("visitCount", std::make_unique<base::Value>(*this->visit_count));

  }
  if (this->typed_count.get()) {
    value->SetWithoutPathExpansion("typedCount", std::make_unique<base::Value>(*this->typed_count));
...
std::unique_ptr<base::DictionaryValue> VisitItem::ToValue() const {
  ...
  value->SetWithoutPathExpansion("visitId", std::make_unique<base::Value>(this->visit_id));

  if (this->visit_time.get()) {
    value->SetWithoutPathExpansion("visitTime", std::make_unique<base::Value>(*this->visit_time));

  }
  value->SetWithoutPathExpansion("referringVisitId", std::make_unique<base::Value>(this->referring_visit_id));

  value->SetWithoutPathExpansion("transition", std::make_unique<base::Value>(history::ToString(this->transition)));
...
std::unique_ptr<base::DictionaryValue> Removed::ToValue() const {
  ...
  value->SetWithoutPathExpansion("allHistory", std::make_unique<base::Value>(this->all_history));

  if (this->urls.get()) {
    value->SetWithoutPathExpansion("urls", json_schema_compiler::util::CreateValueFromOptionalArray(this->urls));
