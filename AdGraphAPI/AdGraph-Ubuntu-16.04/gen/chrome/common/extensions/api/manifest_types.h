...
struct ChromeUIOverrides {
  ChromeUIOverrides();
  ~ChromeUIOverrides();
  ChromeUIOverrides(ChromeUIOverrides&& rhs);
  ChromeUIOverrides& operator=(ChromeUIOverrides&& rhs);

  // Populates a ChromeUIOverrides object from a base::Value. Returns whether
  // |out| was successfully populated.
  static bool Populate(const base::Value& value, ChromeUIOverrides* out, base::string16* error);

  // Creates a ChromeUIOverrides object from a base::Value, or NULL on failure.
  static std::unique_ptr<ChromeUIOverrides> FromValue(const base::Value& value, base::string16* error);

  // Returns a new base::DictionaryValue representing the serialized form of
  // this ChromeUIOverrides object.
  std::unique_ptr<base::DictionaryValue> ToValue() const;
  ...
  struct Bookmarks_ui {
    Bookmarks_ui();
    ~Bookmarks_ui();
    Bookmarks_ui(Bookmarks_ui&& rhs);
    Bookmarks_ui& operator=(Bookmarks_ui&& rhs);

    // Populates a Bookmarks_ui object from a base::Value. Returns whether |out|
    // was successfully populated.
    static bool Populate(const base::Value& value, Bookmarks_ui* out, base::string16* error);

    // Returns a new base::DictionaryValue representing the serialized form of
    // this Bookmarks_ui object.
    std::unique_ptr<base::DictionaryValue> ToValue() const;

    // If <code>true</code>, the built-in bookmark button will be removed from the
    // user interface.
    std::unique_ptr<bool> remove_button;
    ...
    std::unique_ptr<bool> remove_bookmark_shortcut;

    // [Private] If <code>true</code>, the built-in "Bookmark open pages..."
    // shortcut key and corresponding menu item is removed.
    std::unique_ptr<bool> remove_bookmark_open_pages_shortcut;


   private:
    DISALLOW_COPY_AND_ASSIGN(Bookmarks_ui);
  };


  // Settings to permit bookmarks user interface customization by extensions.
  std::unique_ptr<Bookmarks_ui> bookmarks_ui;


 private:
  DISALLOW_COPY_AND_ASSIGN(ChromeUIOverrides);
};
