...
std::unique_ptr<base::DictionaryValue> UpdateStatus::ToValue() const {
  ...
  value->SetWithoutPathExpansion("state", std::make_unique<base::Value>(system_private::ToString(this->state)));

  value->SetWithoutPathExpansion("downloadProgress", std::make_unique<base::Value>(this->download_progress));
