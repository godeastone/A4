...
std::unique_ptr<base::DictionaryValue> OnClickData::ToValue() const {
  ...
  value->SetWithoutPathExpansion("menuItemId", (this->menu_item_id).ToValue());

  if (this->parent_menu_item_id.get()) {
    value->SetWithoutPathExpansion("parentMenuItemId", (this->parent_menu_item_id)->ToValue());

  }
  if (this->media_type.get()) {
    value->SetWithoutPathExpansion("mediaType", std::make_unique<base::Value>(*this->media_type));

  }
  if (this->link_url.get()) {
    value->SetWithoutPathExpansion("linkUrl", std::make_unique<base::Value>(*this->link_url));

  }
  if (this->src_url.get()) {
    value->SetWithoutPathExpansion("srcUrl", std::make_unique<base::Value>(*this->src_url));
  ...
  if (this->frame_url.get()) {
    value->SetWithoutPathExpansion("frameUrl", std::make_unique<base::Value>(*this->frame_url));
  ...
  if (this->selection_text.get()) {
    value->SetWithoutPathExpansion("selectionText", std::make_unique<base::Value>(*this->selection_text));

  }
  value->SetWithoutPathExpansion("editable", std::make_unique<base::Value>(this->editable));

  if (this->was_checked.get()) {
    value->SetWithoutPathExpansion("wasChecked", std::make_unique<base::Value>(*this->was_checked));
