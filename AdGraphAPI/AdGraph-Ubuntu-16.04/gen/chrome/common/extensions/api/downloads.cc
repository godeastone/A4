...
std::unique_ptr<base::DictionaryValue> FilenameSuggestion::ToValue() const {
  ...
  value->SetWithoutPathExpansion("filename", std::make_unique<base::Value>(this->filename));

  if (this->conflict_action != FILENAME_CONFLICT_ACTION_NONE) {
    value->SetWithoutPathExpansion("conflictAction", std::make_unique<base::Value>(downloads::ToString(this->conflict_action)));
...
std::unique_ptr<base::DictionaryValue> DownloadOptions::ToValue() const {
  ...
  if (this->save_as.get()) {
    value->SetWithoutPathExpansion("saveAs", std::make_unique<base::Value>(*this->save_as));

  }
  if (this->method != HTTP_METHOD_NONE) {
    value->SetWithoutPathExpansion("method", std::make_unique<base::Value>(downloads::ToString(this->method)));

  }
  if (this->headers.get()) {
    value->SetWithoutPathExpansion("headers", json_schema_compiler::util::CreateValueFromOptionalArray(this->headers));

  }
  if (this->body.get()) {
    value->SetWithoutPathExpansion("body", std::make_unique<base::Value>(*this->body));
...
std::unique_ptr<base::DictionaryValue> DownloadItem::ToValue() const {
  ...
  value->SetWithoutPathExpansion("finalUrl", std::make_unique<base::Value>(this->final_url));

  value->SetWithoutPathExpansion("referrer", std::make_unique<base::Value>(this->referrer));

  value->SetWithoutPathExpansion("filename", std::make_unique<base::Value>(this->filename));

  value->SetWithoutPathExpansion("incognito", std::make_unique<base::Value>(this->incognito));

  value->SetWithoutPathExpansion("danger", std::make_unique<base::Value>(downloads::ToString(this->danger)));

  value->SetWithoutPathExpansion("mime", std::make_unique<base::Value>(this->mime));

  value->SetWithoutPathExpansion("startTime", std::make_unique<base::Value>(this->start_time));

  if (this->end_time.get()) {
    value->SetWithoutPathExpansion("endTime", std::make_unique<base::Value>(*this->end_time));

  }
  if (this->estimated_end_time.get()) {
    value->SetWithoutPathExpansion("estimatedEndTime", std::make_unique<base::Value>(*this->estimated_end_time));

  }
  value->SetWithoutPathExpansion("state", std::make_unique<base::Value>(downloads::ToString(this->state)));

  value->SetWithoutPathExpansion("paused", std::make_unique<base::Value>(this->paused));

  value->SetWithoutPathExpansion("canResume", std::make_unique<base::Value>(this->can_resume));

  if (this->error != INTERRUPT_REASON_NONE) {
    value->SetWithoutPathExpansion("error", std::make_unique<base::Value>(downloads::ToString(this->error)));

  }
  value->SetWithoutPathExpansion("bytesReceived", std::make_unique<base::Value>(this->bytes_received));

  value->SetWithoutPathExpansion("totalBytes", std::make_unique<base::Value>(this->total_bytes));

  value->SetWithoutPathExpansion("fileSize", std::make_unique<base::Value>(this->file_size));

  value->SetWithoutPathExpansion("exists", std::make_unique<base::Value>(this->exists));

  if (this->by_extension_id.get()) {
    value->SetWithoutPathExpansion("byExtensionId", std::make_unique<base::Value>(*this->by_extension_id));

  }
  if (this->by_extension_name.get()) {
    value->SetWithoutPathExpansion("byExtensionName", std::make_unique<base::Value>(*this->by_extension_name));
...
std::unique_ptr<base::DictionaryValue> DownloadQuery::ToValue() const {
  ...
  if (this->query.get()) {
    value->SetWithoutPathExpansion("query", json_schema_compiler::util::CreateValueFromOptionalArray(this->query));

  }
  if (this->started_before.get()) {
    value->SetWithoutPathExpansion("startedBefore", std::make_unique<base::Value>(*this->started_before));

  }
  if (this->started_after.get()) {
    value->SetWithoutPathExpansion("startedAfter", std::make_unique<base::Value>(*this->started_after));

  }
  if (this->ended_before.get()) {
    value->SetWithoutPathExpansion("endedBefore", std::make_unique<base::Value>(*this->ended_before));

  }
  if (this->ended_after.get()) {
    value->SetWithoutPathExpansion("endedAfter", std::make_unique<base::Value>(*this->ended_after));

  }
  if (this->total_bytes_greater.get()) {
    value->SetWithoutPathExpansion("totalBytesGreater", std::make_unique<base::Value>(*this->total_bytes_greater));

  }
  if (this->total_bytes_less.get()) {
    value->SetWithoutPathExpansion("totalBytesLess", std::make_unique<base::Value>(*this->total_bytes_less));

  }
  if (this->filename_regex.get()) {
    value->SetWithoutPathExpansion("filenameRegex", std::make_unique<base::Value>(*this->filename_regex));

  }
  if (this->url_regex.get()) {
    value->SetWithoutPathExpansion("urlRegex", std::make_unique<base::Value>(*this->url_regex));

  }
  if (this->final_url_regex.get()) {
    value->SetWithoutPathExpansion("finalUrlRegex", std::make_unique<base::Value>(*this->final_url_regex));
  ...
  if (this->order_by.get()) {
    value->SetWithoutPathExpansion("orderBy", json_schema_compiler::util::CreateValueFromOptionalArray(this->order_by));
...
std::unique_ptr<base::DictionaryValue> StringDelta::ToValue() const {
  ...
  if (this->previous.get()) {
    value->SetWithoutPathExpansion("previous", std::make_unique<base::Value>(*this->previous));

  }
  if (this->current.get()) {
    value->SetWithoutPathExpansion("current", std::make_unique<base::Value>(*this->current));
...
std::unique_ptr<base::DictionaryValue> DownloadDelta::ToValue() const {
  ...
  if (this->url.get()) {
    value->SetWithoutPathExpansion("url", (this->url)->ToValue());

  }
  if (this->final_url.get()) {
    value->SetWithoutPathExpansion("finalUrl", (this->final_url)->ToValue());

  }
  if (this->filename.get()) {
    value->SetWithoutPathExpansion("filename", (this->filename)->ToValue());

  }
  if (this->danger.get()) {
    value->SetWithoutPathExpansion("danger", (this->danger)->ToValue());

  }
  if (this->mime.get()) {
    value->SetWithoutPathExpansion("mime", (this->mime)->ToValue());

  }
  if (this->start_time.get()) {
    value->SetWithoutPathExpansion("startTime", (this->start_time)->ToValue());

  }
  if (this->end_time.get()) {
    value->SetWithoutPathExpansion("endTime", (this->end_time)->ToValue());

  }
  if (this->state.get()) {
    value->SetWithoutPathExpansion("state", (this->state)->ToValue());

  }
  if (this->can_resume.get()) {
    value->SetWithoutPathExpansion("canResume", (this->can_resume)->ToValue());

  }
  if (this->paused.get()) {
    value->SetWithoutPathExpansion("paused", (this->paused)->ToValue());

  }
  if (this->error.get()) {
    value->SetWithoutPathExpansion("error", (this->error)->ToValue());

  }
  if (this->total_bytes.get()) {
    value->SetWithoutPathExpansion("totalBytes", (this->total_bytes)->ToValue());

  }
  if (this->file_size.get()) {
    value->SetWithoutPathExpansion("fileSize", (this->file_size)->ToValue());

  }
  if (this->exists.get()) {
    value->SetWithoutPathExpansion("exists", (this->exists)->ToValue());
