...
std::unique_ptr<base::DictionaryValue> PrefObject::ToValue() const {
  ...
  value->SetWithoutPathExpansion("type", std::make_unique<base::Value>(settings_private::ToString(this->type)));

  value->SetWithoutPathExpansion("value", (this->value)->CreateDeepCopy());

  if (this->controlled_by != CONTROLLED_BY_NONE) {
    value->SetWithoutPathExpansion("controlledBy", std::make_unique<base::Value>(settings_private::ToString(this->controlled_by)));

  }
  if (this->controlled_by_name.get()) {
    value->SetWithoutPathExpansion("controlledByName", std::make_unique<base::Value>(*this->controlled_by_name));

  }
  if (this->enforcement != ENFORCEMENT_NONE) {
    value->SetWithoutPathExpansion("enforcement", std::make_unique<base::Value>(settings_private::ToString(this->enforcement)));

  }
  if (this->recommended_value.get()) {
    value->SetWithoutPathExpansion("recommendedValue", (this->recommended_value)->CreateDeepCopy());
  ...
  if (this->extension_can_be_disabled.get()) {
    value->SetWithoutPathExpansion("extensionCanBeDisabled", std::make_unique<base::Value>(*this->extension_can_be_disabled));
...
std::unique_ptr<Params> Params::Create(const base::ListValue& args) {
  ...
  if (args.Get(2, &page_id_value) &&
    ...
    {
      if (!page_id_value->GetAsString(&params->page_id)) {
        return std::unique_ptr<Params>();
      }
    }
  }
