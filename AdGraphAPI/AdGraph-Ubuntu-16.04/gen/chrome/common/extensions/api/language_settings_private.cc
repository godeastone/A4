...
is_allowed_locale(std::move(rhs.is_allowed_locale)){
}
...
bool Language::Populate(
  ...
  const base::Value* is_allowed_locale_value = NULL;
  if (dict->GetWithoutPathExpansion("isAllowedLocale", &is_allowed_locale_value)) {
    {
      bool temp;
      if (!is_allowed_locale_value->GetAsBoolean(&temp)) {
        out->is_allowed_locale.reset();
        return false;
      }
      else
        out->is_allowed_locale.reset(new bool(temp));
    }
  }
...
std::unique_ptr<base::DictionaryValue> Language::ToValue() const {
  ...
  value->SetWithoutPathExpansion("nativeDisplayName", std::make_unique<base::Value>(this->native_display_name));

  if (this->supports_ui.get()) {
    value->SetWithoutPathExpansion("supportsUI", std::make_unique<base::Value>(*this->supports_ui));

  }
  if (this->supports_spellcheck.get()) {
    value->SetWithoutPathExpansion("supportsSpellcheck", std::make_unique<base::Value>(*this->supports_spellcheck));

  }
  if (this->supports_translate.get()) {
    value->SetWithoutPathExpansion("supportsTranslate", std::make_unique<base::Value>(*this->supports_translate));

  }
  if (this->is_allowed_locale.get()) {
    value->SetWithoutPathExpansion("isAllowedLocale", std::make_unique<base::Value>(*this->is_allowed_locale));
...
std::unique_ptr<base::DictionaryValue> SpellcheckDictionaryStatus::ToValue() const {
  ...
  if (this->is_downloading.get()) {
    value->SetWithoutPathExpansion("isDownloading", std::make_unique<base::Value>(*this->is_downloading));

  }
  if (this->download_failed.get()) {
    value->SetWithoutPathExpansion("downloadFailed", std::make_unique<base::Value>(*this->download_failed));
...
std::unique_ptr<base::DictionaryValue> InputMethod::ToValue() const {
  ...
  value->SetWithoutPathExpansion("languageCodes", json_schema_compiler::util::CreateValueFromArray(this->language_codes));
  ...
  if (this->has_options_page.get()) {
    value->SetWithoutPathExpansion("hasOptionsPage", std::make_unique<base::Value>(*this->has_options_page));
...
std::unique_ptr<base::DictionaryValue> InputMethodLists::ToValue() const {
  ...
  value->SetWithoutPathExpansion("componentExtensionImes", json_schema_compiler::util::CreateValueFromArray(this->component_extension_imes));

  value->SetWithoutPathExpansion("thirdPartyExtensionImes", json_schema_compiler::util::CreateValueFromArray(this->third_party_extension_imes));
