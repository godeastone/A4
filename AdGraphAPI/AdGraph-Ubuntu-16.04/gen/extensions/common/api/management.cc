...
std::unique_ptr<base::DictionaryValue> ExtensionInfo::ToValue() const {
  ...
  if (this->version_name.get()) {
    value->SetWithoutPathExpansion("versionName", std::make_unique<base::Value>(*this->version_name));

  }
  value->SetWithoutPathExpansion("mayDisable", std::make_unique<base::Value>(this->may_disable));

  if (this->may_enable.get()) {
    value->SetWithoutPathExpansion("mayEnable", std::make_unique<base::Value>(*this->may_enable));
  ...
  if (this->disabled_reason != EXTENSION_DISABLED_REASON_NONE) {
    value->SetWithoutPathExpansion("disabledReason", std::make_unique<base::Value>(management::ToString(this->disabled_reason)));

  }
  value->SetWithoutPathExpansion("isApp", std::make_unique<base::Value>(this->is_app));

  value->SetWithoutPathExpansion("type", std::make_unique<base::Value>(management::ToString(this->type)));

  if (this->app_launch_url.get()) {
    value->SetWithoutPathExpansion("appLaunchUrl", std::make_unique<base::Value>(*this->app_launch_url));

  }
  if (this->homepage_url.get()) {
    value->SetWithoutPathExpansion("homepageUrl", std::make_unique<base::Value>(*this->homepage_url));
  ...
  if (this->icons.get()) {
    value->SetWithoutPathExpansion("icons", json_schema_compiler::util::CreateValueFromOptionalArray(this->icons));
  ...
  value->SetWithoutPathExpansion("installType", std::make_unique<base::Value>(management::ToString(this->install_type)));

  if (this->launch_type != LAUNCH_TYPE_NONE) {
    value->SetWithoutPathExpansion("launchType", std::make_unique<base::Value>(management::ToString(this->launch_type)));
  ...
  if (this->available_launch_types.get()) {
    ...
    value->SetWithoutPathExpansion("availableLaunchTypes", json_schema_compiler::util::CreateValueFromArray(availableLaunchTypes_list));
    }
