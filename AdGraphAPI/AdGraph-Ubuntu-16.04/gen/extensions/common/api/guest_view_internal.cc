...
std::unique_ptr<base::DictionaryValue> SizeParams::ToValue() const {
  ...
  if (this->enable_auto_size.get()) {
    value->SetWithoutPathExpansion("enableAutoSize", std::make_unique<base::Value>(*this->enable_auto_size));

  }
  if (this->min.get()) {
    value->SetWithoutPathExpansion("min", (this->min)->ToValue());

  }
  if (this->max.get()) {
    value->SetWithoutPathExpansion("max", (this->max)->ToValue());

  }
  if (this->normal.get()) {
    value->SetWithoutPathExpansion("normal", (this->normal)->ToValue());
