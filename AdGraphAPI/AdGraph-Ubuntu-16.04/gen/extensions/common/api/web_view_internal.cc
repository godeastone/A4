...
std::unique_ptr<base::DictionaryValue> DataTypeSet::ToValue() const {
  ...
  if (this->appcache.get()) {
    value->SetWithoutPathExpansion("appcache", std::make_unique<base::Value>(*this->appcache));

  }
  if (this->cookies.get()) {
    value->SetWithoutPathExpansion("cookies", std::make_unique<base::Value>(*this->cookies));

  }
  if (this->session_cookies.get()) {
    value->SetWithoutPathExpansion("sessionCookies", std::make_unique<base::Value>(*this->session_cookies));

  }
  if (this->persistent_cookies.get()) {
    value->SetWithoutPathExpansion("persistentCookies", std::make_unique<base::Value>(*this->persistent_cookies));

  }
  if (this->file_systems.get()) {
    value->SetWithoutPathExpansion("fileSystems", std::make_unique<base::Value>(*this->file_systems));

  }
  if (this->indexed_db.get()) {
    value->SetWithoutPathExpansion("indexedDB", std::make_unique<base::Value>(*this->indexed_db));

  }
  if (this->local_storage.get()) {
    value->SetWithoutPathExpansion("localStorage", std::make_unique<base::Value>(*this->local_storage));

  }
  if (this->web_sql.get()) {
    value->SetWithoutPathExpansion("webSQL", std::make_unique<base::Value>(*this->web_sql));

  }
  if (this->cache.get()) {
    value->SetWithoutPathExpansion("cache", std::make_unique<base::Value>(*this->cache));
...
std::unique_ptr<base::DictionaryValue> RemovalOptions::ToValue() const {
  ...
  if (this->since.get()) {
    value->SetWithoutPathExpansion("since", std::make_unique<base::Value>(*this->since));
...
std::unique_ptr<base::DictionaryValue> InjectionItems::ToValue() const {
  ...
  if (this->files.get()) {
    value->SetWithoutPathExpansion("files", json_schema_compiler::util::CreateValueFromOptionalArray(this->files));
...
std::unique_ptr<base::DictionaryValue> ContentScriptDetails::ToValue() const {
  ...
  if (this->exclude_matches.get()) {
    value->SetWithoutPathExpansion("exclude_matches", json_schema_compiler::util::CreateValueFromOptionalArray(this->exclude_matches));

  }
  if (this->match_about_blank.get()) {
    value->SetWithoutPathExpansion("match_about_blank", std::make_unique<base::Value>(*this->match_about_blank));

  }
  if (this->css.get()) {
    value->SetWithoutPathExpansion("css", (this->css)->ToValue());

  }
  if (this->js.get()) {
    value->SetWithoutPathExpansion("js", (this->js)->ToValue());

  }
  if (this->run_at != extension_types::RUN_AT_NONE) {
    value->SetWithoutPathExpansion("run_at", std::make_unique<base::Value>(extension_types::ToString(this->run_at)));

  }
  if (this->all_frames.get()) {
    value->SetWithoutPathExpansion("all_frames", std::make_unique<base::Value>(*this->all_frames));

  }
  if (this->include_globs.get()) {
    value->SetWithoutPathExpansion("include_globs", json_schema_compiler::util::CreateValueFromOptionalArray(this->include_globs));

  }
  if (this->exclude_globs.get()) {
    value->SetWithoutPathExpansion("exclude_globs", json_schema_compiler::util::CreateValueFromOptionalArray(this->exclude_globs));
...
std::unique_ptr<base::DictionaryValue> Results::Results::ToValue() const {
  ...
  value->SetWithoutPathExpansion("numberOfMatches", std::make_unique<base::Value>(this->number_of_matches));

  value->SetWithoutPathExpansion("activeMatchOrdinal", std::make_unique<base::Value>(this->active_match_ordinal));

  value->SetWithoutPathExpansion("selectionRect", (this->selection_rect).ToValue());

  value->SetWithoutPathExpansion("canceled", std::make_unique<base::Value>(this->canceled));
