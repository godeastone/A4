...
std::unique_ptr<base::DictionaryValue> Rule::ToValue() const {
  ...
  if (this->tags.get()) {
    value->SetWithoutPathExpansion("tags", json_schema_compiler::util::CreateValueFromOptionalArray(this->tags));

  }
  value->SetWithoutPathExpansion("conditions", json_schema_compiler::util::CreateValueFromArray(this->conditions));

  value->SetWithoutPathExpansion("actions", json_schema_compiler::util::CreateValueFromArray(this->actions));
