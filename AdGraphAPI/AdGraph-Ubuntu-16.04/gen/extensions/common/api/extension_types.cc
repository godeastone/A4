...
std::unique_ptr<base::DictionaryValue> ImageDetails::ToValue() const {
  ...
  if (this->format != IMAGE_FORMAT_NONE) {
    value->SetWithoutPathExpansion("format", std::make_unique<base::Value>(extension_types::ToString(this->format)));

  }
  if (this->quality.get()) {
    value->SetWithoutPathExpansion("quality", std::make_unique<base::Value>(*this->quality));
...
std::unique_ptr<base::DictionaryValue> InjectDetails::ToValue() const {
  ...
  if (this->all_frames.get()) {
    value->SetWithoutPathExpansion("allFrames", std::make_unique<base::Value>(*this->all_frames));
  ...
  if (this->match_about_blank.get()) {
    value->SetWithoutPathExpansion("matchAboutBlank", std::make_unique<base::Value>(*this->match_about_blank));

  }
  if (this->run_at != RUN_AT_NONE) {
    value->SetWithoutPathExpansion("runAt", std::make_unique<base::Value>(extension_types::ToString(this->run_at)));

  }
  if (this->css_origin != CSS_ORIGIN_NONE) {
    value->SetWithoutPathExpansion("cssOrigin", std::make_unique<base::Value>(extension_types::ToString(this->css_origin)));
