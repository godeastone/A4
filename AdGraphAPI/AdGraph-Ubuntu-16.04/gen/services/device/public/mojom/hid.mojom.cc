...
HidCollectionInfo::HidCollectionInfo(
    ...
    const std::vector<int32_t>& report_ids_in)
    : usage(std::move(usage_in)),
...
HidDeviceInfo::HidDeviceInfo(
    ...
    const std::vector<uint8_t>& report_descriptor_in,
...
void HidManagerProxy::Connect(
    const std::string& in_device_guid, ConnectCallback callback) {
#if BUILDFLAG(MOJO_TRACE_ENABLED)
...
void HidManagerInterceptorForTesting::Connect(const std::string& device_guid, ConnectCallback callback) {
...
class HidConnection_Read_ProxyToResponder {
 ...
 private:
  ...
  void Run(
      bool in_success, uint8_t in_report_id, const base::Optional<std::vector<uint8_t>>& in_buffer);
...
class HidConnection_GetFeatureReport_ProxyToResponder {
 ...
 private:
  ...
  void Run(
      bool in_success, const base::Optional<std::vector<uint8_t>>& in_buffer);
