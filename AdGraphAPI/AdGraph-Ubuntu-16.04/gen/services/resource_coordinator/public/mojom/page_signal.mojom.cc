...
void PageSignalReceiverProxy::SetExpectedTaskQueueingDuration(
    const ::resource_coordinator::CoordinationUnitID& in_cu_id, base::TimeDelta in_duration) {
#if BUILDFLAG(MOJO_TRACE_ENABLED)
...
void PageSignalReceiverProxy::SetLifecycleState(
    const ::resource_coordinator::CoordinationUnitID& in_cu_id, ::resource_coordinator::mojom::LifecycleState in_state) {
#if BUILDFLAG(MOJO_TRACE_ENABLED)
...
void PageSignalReceiverInterceptorForTesting::NotifyPageAlmostIdle(const ::resource_coordinator::CoordinationUnitID& cu_id) {
  GetForwardingInterface()->NotifyPageAlmostIdle(std::move(cu_id));
}
void PageSignalReceiverInterceptorForTesting::SetExpectedTaskQueueingDuration(const ::resource_coordinator::CoordinationUnitID& cu_id, base::TimeDelta duration) {
  GetForwardingInterface()->SetExpectedTaskQueueingDuration(std::move(cu_id), std::move(duration));
}
void PageSignalReceiverInterceptorForTesting::SetLifecycleState(const ::resource_coordinator::CoordinationUnitID& cu_id, ::resource_coordinator::mojom::LifecycleState state) {
