...
class ClientProcess_EnableHeapProfiling_ForwardToCallback
    : public mojo::MessageReceiver {
 public:
  ClientProcess_EnableHeapProfiling_ForwardToCallback(
      ClientProcess::EnableHeapProfilingCallback callback
      ) : callback_(std::move(callback)) {
  }
  bool Accept(mojo::Message* message) override;
 private:
  ClientProcess::EnableHeapProfilingCallback callback_;
  DISALLOW_COPY_AND_ASSIGN(ClientProcess_EnableHeapProfiling_ForwardToCallback);
};
...
void ClientProcessProxy::EnableHeapProfiling(
    base::trace_event::HeapProfilingMode in_mode, EnableHeapProfilingCallback callback) {
#if BUILDFLAG(MOJO_TRACE_ENABLED)
  TRACE_EVENT0("mojom", "memory_instrumentation::mojom::ClientProcess::EnableHeapProfiling");
#endif
  const bool kExpectsResponse = true;
  const bool kIsSync = false;
  ...
  mojo::Message message(
      internal::kClientProcess_EnableHeapProfiling_Name, kFlags, 0, 0, nullptr);
  auto* buffer = message.payload_buffer();
  ::memory_instrumentation::mojom::internal::ClientProcess_EnableHeapProfiling_Params_Data::BufferWriter
      params;
  mojo::internal::SerializationContext serialization_context;
  params.Allocate(buffer);
  mojo::internal::Serialize<::memory_instrumentation::mojom::HeapProfilingMode>(
      in_mode, &params->mode);
  message.AttachHandlesFromSerializationContext(
      &serialization_context);
  std::unique_ptr<mojo::MessageReceiver> responder(
      new ClientProcess_EnableHeapProfiling_ForwardToCallback(
          std::move(callback)));
  ignore_result(receiver_->AcceptWithResponder(&message, std::move(responder)));
}
...
class ClientProcess_EnableHeapProfiling_ProxyToResponder {
 public:
  static ClientProcess::EnableHeapProfilingCallback CreateCallback(
      uint64_t request_id,
      bool is_sync,
      std::unique_ptr<mojo::MessageReceiverWithStatus> responder) {
    std::unique_ptr<ClientProcess_EnableHeapProfiling_ProxyToResponder> proxy(
        new ClientProcess_EnableHeapProfiling_ProxyToResponder(
            request_id, is_sync, std::move(responder)));
    return base::BindOnce(&ClientProcess_EnableHeapProfiling_ProxyToResponder::Run,
                          std::move(proxy));
  }

  ~ClientProcess_EnableHeapProfiling_ProxyToResponder() {
#if DCHECK_IS_ON()
    if (responder_) {
      // If we're being destroyed without being run, we want to ensure the
      // binding endpoint has been closed. This checks for that asynchronously.
 ...
 private:
  ...
  static void OnIsConnectedComplete(bool connected) {
    DCHECK(!connected)
        << "ClientProcess::EnableHeapProfilingCallback was destroyed without "
        << "first either being run or its corresponding binding being closed. "
        << "It is an error to drop response callbacks which still correspond "
        << "to an open interface pipe.";
  }
  ...
  DISALLOW_COPY_AND_ASSIGN(ClientProcess_EnableHeapProfiling_ProxyToResponder);
};

bool ClientProcess_EnableHeapProfiling_ForwardToCallback::Accept(
    mojo::Message* message) {
#if BUILDFLAG(MOJO_TRACE_ENABLED)
  TRACE_EVENT1("mojom", "memory_instrumentation::mojom::ClientProcess::EnableHeapProfilingCallback",
               "message", message->name());
#endif
  mojo::internal::MessageDispatchContext dispatch_context(message);

  DCHECK(message->is_serialized());
  internal::ClientProcess_EnableHeapProfiling_ResponseParams_Data* params =
      reinterpret_cast<
          internal::ClientProcess_EnableHeapProfiling_ResponseParams_Data*>(
              message->mutable_payload());
  ...
  bool success = true;
  bool p_success{};
  ClientProcess_EnableHeapProfiling_ResponseParamsDataView input_data_view(params, &serialization_context);
  ...
  if (!success) {
    ReportValidationErrorForMessage(
        message,
        mojo::internal::VALIDATION_ERROR_DESERIALIZATION_FAILED,
        "ClientProcess::EnableHeapProfiling response deserializer");
    return false;
  }
  if (!callback_.is_null())
    std::move(callback_).Run(
...
void ClientProcess_EnableHeapProfiling_ProxyToResponder::Run(
    bool in_success) {
  ...
  mojo::Message message(
      internal::kClientProcess_EnableHeapProfiling_Name, kFlags, 0, 0, nullptr);
  auto* buffer = message.payload_buffer();
  ::memory_instrumentation::mojom::internal::ClientProcess_EnableHeapProfiling_ResponseParams_Data::BufferWriter
      params;
  mojo::internal::SerializationContext serialization_context;
  params.Allocate(buffer);
  params->success = in_success;
...
#if BUILDFLAG(MOJO_TRACE_ENABLED)
  TRACE_EVENT1("mojom", "(Impl)memory_instrumentation::mojom::ClientProcess::EnableHeapProfilingCallback",
               "message", message.name());
#endif
  message.set_request_id(request_id_);
  ignore_result(responder_->Accept(&message));
...
bool ClientProcessStubDispatch::Accept(
  ...
  TRACE_EVENT1("mojom", "(Impl)memory_instrumentation::mojom::ClientProcess::RequestChromeMemoryDump",
    ...
    case internal::kClientProcess_EnableHeapProfiling_Name: {
#if BUILDFLAG(MOJO_TRACE_ENABLED)
  TRACE_EVENT1("mojom", "(Impl)memory_instrumentation::mojom::ClientProcess::EnableHeapProfiling",
               "message", message->name());
#endif
      break;
    }
...
std::move(p_args), std::move(callback));
  ...
  TRACE_EVENT1("mojom", "(Impl)memory_instrumentation::mojom::ClientProcess::EnableHeapProfiling",
      ...
      internal::ClientProcess_EnableHeapProfiling_Params_Data* params =
          reinterpret_cast<
              internal::ClientProcess_EnableHeapProfiling_Params_Data*>(
                  message->mutable_payload());
      
      mojo::internal::SerializationContext serialization_context;
      serialization_context.TakeHandlesFromMessage(message);
      bool success = true;
      base::trace_event::HeapProfilingMode p_mode{};
      ClientProcess_EnableHeapProfiling_ParamsDataView input_data_view(params, &serialization_context);
      ...
      if (!success) {
        ReportValidationErrorForMessage(
            message,
            mojo::internal::VALIDATION_ERROR_DESERIALIZATION_FAILED,
            "ClientProcess::EnableHeapProfiling deserializer");
        return false;
      }
      ClientProcess::EnableHeapProfilingCallback callback =
          ClientProcess_EnableHeapProfiling_ProxyToResponder::CreateCallback(
              message->request_id(),
              message->has_flag(mojo::Message::kFlagIsSync),
              std::move(responder));
      // A null |impl| means no implementation was bound.
      assert(impl);
      impl->EnableHeapProfiling(
...
void ClientProcessInterceptorForTesting::EnableHeapProfiling(base::trace_event::HeapProfilingMode mode, EnableHeapProfilingCallback callback) {
  GetForwardingInterface()->EnableHeapProfiling(std::move(mode), std::move(callback));
}
...
void ClientProcessAsyncWaiter::EnableHeapProfiling(
    base::trace_event::HeapProfilingMode mode, bool* out_success) {
  base::RunLoop loop;
  proxy_->EnableHeapProfiling(std::move(mode),
      base::BindOnce(
          [](base::RunLoop* loop,
             bool* out_success
,
