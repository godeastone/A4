...
void AgentRegistryProxy::RegisterAgent(
    AgentPtr in_agent, const std::string& in_label, TraceDataType in_type, bool in_supports_explicit_clock_sync_) {
  ...
  mojo::internal::Serialize<::tracing::mojom::TraceDataType>(
      in_type, &params->type);
  params->supports_explicit_clock_sync_ = in_supports_explicit_clock_sync_;
...
bool AgentRegistryStubDispatch::Accept(
  ...
  TRACE_EVENT1("mojom", "(Impl)tracing::mojom::AgentRegistry::RegisterAgent",
      ...
      bool p_supports_explicit_clock_sync_{};
      ...
      if (!input_data_view.ReadType(&p_type))
        success = false;
      p_supports_explicit_clock_sync_ = input_data_view.supports_explicit_clock_sync_();
      if (!success) {
...
std::move(p_label), 
std::move(p_type), 
std::move(p_supports_explicit_clock_sync_));
      return true;
    }
  }
  return false;
...
void AgentRegistryInterceptorForTesting::RegisterAgent(AgentPtr agent, const std::string& label, TraceDataType type, bool supports_explicit_clock_sync_) {
  GetForwardingInterface()->RegisterAgent(std::move(agent), std::move(label), std::move(type), std::move(supports_explicit_clock_sync_));
}
